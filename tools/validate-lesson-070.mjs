...
const component = readFileSync(resolve(root, 'plataforma-curso/src/components/GuidedConversionsCastingLesson070.jsx'), 'utf8');
const css = readFileSync(resolve(root, 'plataforma-curso/src/components/guidedConversionsCastingLesson.css'), 'utf8');
const viewer = readFileSync(resolve(root, 'plataforma-curso/src/components/MarkdownViewer.jsx'), 'utf8');
const matrix = readFileSync(resolve(root, 'docs/revisao-aulas/matrizes/070_CONVERSOES_CASTING.md'), 'utf8');
const must = (condition, message) => { if (!condition) throw new Error(`Aula 070: ${message}`); };

must(viewer.includes("startsWith('070_')") && viewer.includes('GuidedConversionsCastingLesson070'), 'integração ausente');
...
must(component.includes('Integer.MIN_VALUE') && component.includes('Integer.MAX_VALUE'), 'validação de faixa ausente');
must(component.includes('NumberFormatException') && component.includes('Double.parseDouble') && component.includes('Integer.valueOf'), 'bancada de parsing incompleta');
must(component.includes('Boolean.parseBoolean não é validador'), 'boolean permissivo não explicado');
...
must(css.includes('.cv70-errors button>span:first-child') && css.includes('.cv70-errors button>span:last-child'), 'seletores da Clínica incorretos');
...
const temp = mkdtempSync(join(tmpdir(), 'java-lesson-070-'));
try {
  const file = join(temp, 'LaboratorioConversoes.java');
  ...
  const run = spawnSync('java', ['-cp', temp, 'LaboratorioConversoes'], { encoding: 'utf8' });
...
console.log('LESSON_070_STATIC_JAVA_AND_OUTPUT_OK');
