...
const component = readFileSync(resolve(root, 'plataforma-curso/src/components/GuidedPatternMatchingLesson083.jsx'), 'utf8');
const css = readFileSync(resolve(root, 'plataforma-curso/src/components/guidedPatternMatchingLesson.css'), 'utf8');
const viewer = readFileSync(resolve(root, 'plataforma-curso/src/components/MarkdownViewer.jsx'), 'utf8');
const matrix = readFileSync(resolve(root, 'docs/revisao-aulas/matrizes/083_PATTERN_MATCHING.md'), 'utf8');
const must = (condition, message) => { if (!condition) throw new Error('Aula 083: ' + message); };

must(viewer.includes("startsWith('083_')") && viewer.includes('GuidedPatternMatchingLesson083'), 'integração ausente');
...
must(component.includes('instanceof String texto') && component.includes('instanceof PedidoResumo resumo'), 'type patterns incompletos');
must(component.includes('&& resumo.status()') && component.includes('instanceof String texto ||'), 'lógica &&/|| incompleta');
must(component.includes('null combina: false') && component.includes('Flow scoping'), 'null ou flow scoping ausente');
must(component.includes('switch (pagamento)') && component.includes('Prefira polimorfismo'), 'switch conceitual ou decisão de design ausente');
...
must(matrix.includes('mantendo integralmente') && matrix.includes('Aula 084'), 'matriz incompleta');
...
const temp = mkdtempSync(join(tmpdir(), 'java-lesson-083-'));
try {
  const file = join(temp, 'LaboratorioPatternMatching.java');
  ...
  const run = spawnSync('java', ['-cp', temp, 'LaboratorioPatternMatching'], { encoding: 'utf8' });
...
console.log('LESSON_083_STATIC_JAVA_AND_OUTPUT_OK');
