...
const component = readFileSync(resolve(root, 'plataforma-curso/src/components/GuidedNullPointerLesson066.jsx'), 'utf8');
const css = readFileSync(resolve(root, 'plataforma-curso/src/components/guidedNullPointerLesson.css'), 'utf8');
const viewer = readFileSync(resolve(root, 'plataforma-curso/src/components/MarkdownViewer.jsx'), 'utf8');
const matrix = readFileSync(resolve(root, 'docs/revisao-aulas/matrizes/066_NULL_NULLPOINTEREXCEPTION.md'), 'utf8');
const must = (condition, message) => { if (!condition) throw new Error(`Aula 066: ${message}`); };

must(viewer.includes("startsWith('066_')") && viewer.includes('GuidedNullPointerLesson066'), 'integração ausente');
...
must(component.includes('null significa que uma referência não aponta para objeto'), 'contrato central ausente');
must(component.includes('Objects.requireNonNull') && component.includes('Optional.empty()'), 'fail fast ou Optional ausente');
must(component.includes('className="guided-error-label"'), 'Clínica fora do padrão');
must((component.slice(component.indexOf('const steps = ['), component.indexOf('export default function')).match(/\bid: '/g) || []).length === 10, 'roteiro não contém dez etapas');
...
must(css.includes('.np66-errors button>span:first-child') && css.includes('.np66-errors button>span:last-child'), 'seletores da Clínica incorretos');
...
const temp = mkdtempSync(join(tmpdir(), 'java-lesson-066-'));
try {
  const file = join(temp, 'LaboratorioNullSeguro.java');
  ...
  const run = spawnSync('java', ['-cp', temp, 'LaboratorioNullSeguro'], { encoding: 'utf8' });
...
console.log('LESSON_066_STATIC_JAVA_AND_OUTPUT_OK');
