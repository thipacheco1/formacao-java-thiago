...
const component = readFileSync(resolve(root, 'plataforma-curso/src/components/GuidedRecordsLesson077.jsx'), 'utf8');
const css = readFileSync(resolve(root, 'plataforma-curso/src/components/guidedRecordsLesson.css'), 'utf8');
const viewer = readFileSync(resolve(root, 'plataforma-curso/src/components/MarkdownViewer.jsx'), 'utf8');
const matrix = readFileSync(resolve(root, 'docs/revisao-aulas/matrizes/077_RECORDS.md'), 'utf8');
const must = (condition, message) => { if (!condition) throw new Error('Aula 077: ' + message); };

must(viewer.includes("startsWith('077_')") && viewer.includes('GuidedRecordsLesson077'), 'integração ausente');
...
must(component.includes('record ClienteResumo') && component.includes('record PedidoResumo'), 'records centrais ausentes');
must(component.includes('Objects.requireNonNull') && component.includes('public ClienteResumo {'), 'validação compacta ausente');
must(component.includes('equals') && component.includes('hashCode') && component.includes('toString'), 'contrato gerado incompleto');
must(component.includes('BigDecimal') && component.includes('Instant') && component.includes('implements Identificavel'), 'composição profissional incompleta');
...
const temp = mkdtempSync(join(tmpdir(), 'java-lesson-077-'));
try {
  const file = join(temp, 'LaboratorioRecords.java');
  ...
  const run = spawnSync('java', ['-cp', temp, 'LaboratorioRecords'], { encoding: 'utf8' });
...
console.log('LESSON_077_STATIC_JAVA_AND_OUTPUT_OK');
