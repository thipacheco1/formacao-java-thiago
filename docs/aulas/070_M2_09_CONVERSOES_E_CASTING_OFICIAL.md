...
Agora vamos aprofundar conversões.

Conversão acontece quando um valor de um tipo precisa virar outro tipo.
...
Esses três exemplos parecem simples, mas escondem riscos importantes:
...
uso errado de cast;
conversão automática sem perceber;
conversão explícita perigosa;
parsing sem validação;
regra de negócio mascarada por conversão.
```
...
## Diagnóstico de conversões

Quando uma conversão der problema, pergunte:

### 1. Qual é o tipo de origem?
...
### 3. É widening ou narrowing?

Widening tende a ser automático.

Narrowing exige cuidado.

### 4. Pode perder precisão?
...
### 6. O texto está em formato válido?
...
"123" ok;
"abc" inválido;
"10.5" inválido para int.
```

### 7. O texto tem espaços?

Use `trim`.

### 8. O erro deve virar valor padrão ou exceção?

Depende da regra.

### 9. NumberFormatException está sendo tratada?

Se entrada é externa, trate.

### 10. Use debug

Veja o valor antes da conversão.
...
Explique truncamento.

---

### Teste 3 — divisão inteira
...
### Teste 5 — Boolean.parseBoolean

```java
public class Main {
    public static void main(String[] args) {
        boolean valor = Boolean.parseBoolean("abc");
...
Explique por que retorna `false` e por que isso pode ser perigoso.
...
```markdown
# Aula 070 — Conversões e casting

## Objetivo

Entender conversões entre tipos em Java, incluindo widening, narrowing, cast explícito, parsing de texto, `parseInt`, `valueOf`, `NumberFormatException` e validação.

## Conceitos

- Widening é conversão para tipo mais amplo.
- Narrowing é conversão para tipo mais restrito.
- Cast explícito força conversão, mas não valida regra.
- Cast pode causar perda de precisão.
- Cast de `double` para `int` trunca, não arredonda.
- `long` grande para `int` pode causar overflow.
- `Integer.parseInt` converte texto para `int`.
- `Long.parseLong` converte texto para `long`.
- `Double.parseDouble` converte texto para `double`.
- `valueOf` retorna wrapper.
- Texto inválido gera `NumberFormatException`.
- Entrada externa precisa validação.
- Converter e validar regra são responsabilidades diferentes.
...
| Debug | `Shift + F9` | Ver valor antes da conversão |
| Run | `Shift + F10` | Executar normal |
| Step Over | `F8` em muitos keymaps | Avançar conversão |
| Variables | janela Debug | Ver origem e destino |
| Evaluate Expression | `Alt + F8` em muitos keymaps | Testar casts e parsing |
| Reformatar código | `Ctrl + Alt + L` | Organizar |
| Renomear | `Shift + F6` | Melhorar nomes |
| Extrair método | `Ctrl + Alt + M` em muitos keymaps | Centralizar conversão |
...
### O que aprendi
Aprendi que conversões em Java podem ser implícitas ou explícitas, que widening geralmente é automático, que narrowing exige cuidado, que cast pode causar perda de precisão ou overflow, e que parsing de texto pode lançar `NumberFormatException`.

### O que pratiquei
Criei exemplos com widening, narrowing, cast de `long` para `int`, cast de `double` para `int`, divisão inteira, `parseInt`, `parseLong`, `parseDouble`, `valueOf`, `NumberFormatException`, validação antes de converter e aplicações em cliente, produto, pedido, pagamento, OS, mensageria e auditoria.
...
### Arquivos criados
- `labs/m2/aula-070-conversoes-casting/Main.java`
- `labs/m2/aula-070-conversoes-casting/WideningNumerico.java`
- `labs/m2/aula-070-conversoes-casting/NarrowingComCast.java`
- `labs/m2/aula-070-conversoes-casting/OverflowNarrowing.java`
- `labs/m2/aula-070-conversoes-casting/LongParaIntSeguro.java`
- `labs/m2/aula-070-conversoes-casting/DoubleParaInt.java`
- `labs/m2/aula-070-conversoes-casting/CastNaoArredonda.java`
- `labs/m2/aula-070-conversoes-casting/CharParaInt.java`
- `labs/m2/aula-070-conversoes-casting/PromocaoExpressao.java`
- `labs/m2/aula-070-conversoes-casting/DivisaoInteira.java`
- `labs/m2/aula-070-conversoes-casting/ParseIntExemplo.java`
- `labs/m2/aula-070-conversoes-casting/ParseLongExemplo.java`
- `labs/m2/aula-070-conversoes-casting/ParseDoubleExemplo.java`
- `labs/m2/aula-070-conversoes-casting/ValueOfExemplo.java`
- `labs/m2/aula-070-conversoes-casting/ErroNumberFormatException.java`
- `labs/m2/aula-070-conversoes-casting/ParseComTryCatch.java`
- `labs/m2/aula-070-conversoes-casting/ParseInteiroSeguro.java`
- `labs/m2/aula-070-conversoes-casting/ParseInteiroComPadrao.java`
- `labs/m2/aula-070-conversoes-casting/ParseInteiroObrigatorio.java`
- `labs/m2/aula-070-conversoes-casting/ValidarDigitosAntesParse.java`
- `labs/m2/aula-070-conversoes-casting/ConverterEValidarQuantidade.java`
- `labs/m2/aula-070-conversoes-casting/ClienteConversao.java`
- `labs/m2/aula-070-conversoes-casting/ProdutoConversao.java`
- `labs/m2/aula-070-conversoes-casting/PedidoConversao.java`
- `labs/m2/aula-070-conversoes-casting/PagamentoConversao.java`
- `labs/m2/aula-070-conversoes-casting/OrdemServicoConversao.java`
- `labs/m2/aula-070-conversoes-casting/MensageriaConversao.java`
- `labs/m2/aula-070-conversoes-casting/AuditoriaConversao.java`
- `labs/m2/aula-070-conversoes-casting/DebugConversao.java`
- `labs/m2/aula-070-conversoes-casting/ErroCastOverflow.java`
- `labs/m2/aula-070-conversoes-casting/ErroCastArredondamento.java`
- `labs/m2/aula-070-conversoes-casting/ErroDivisaoInteira.java`
- `labs/m2/aula-070-conversoes-casting/ErroBooleanParse.java`
- `labs/m2/aula-070-conversoes-casting/README.md`
...
- achar que cast valida faixa;
- achar que cast arredonda;
- converter long grande para int sem validar;
- fazer divisão inteira sem perceber;
- usar parseInt em decimal;
- não tratar NumberFormatException;
- usar Boolean.parseBoolean achando que valida;
- converter null sem validar;
- retornar 0 para todo parse inválido;
- misturar conversão com regra sem clareza.

### Próximo passo
Estudar Math, Random e números utilitários.
```
...
4. O que é cast?
5. Cast valida faixa?
6. Cast de double para int arredonda?
7. O que é truncamento?
8. O que é overflow?
9. Por que long grande para int pode dar problema?
10. Por que double resultado = 10 / 4 gera 2.0?
11. Para que serve Integer.parseInt?
12. Para que serve Long.parseLong?
13. Para que serve Double.parseDouble?
14. Qual a diferença entre parseInt e valueOf?
15. O que é NumberFormatException?
16. Por que validar texto antes ou durante parse?
17. Quando usar valor padrão em parse?
18. Quando lançar erro em parse?
19. Por que Boolean.parseBoolean pode ser perigoso?
20. Por que converter e validar são responsabilidades diferentes?
```
