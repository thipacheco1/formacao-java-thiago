...
recalcular resultado após alteração.
```

Agora vem uma habilidade essencial:
...
Essa estrutura aparece o tempo todo em programação.

Mesmo quando, no futuro, usarmos listas, banco de dados, APIs, repositórios e streams, a lógica mental de busca continua sendo importante.
...
## Diagnóstico de busca em array

Quando a busca não funcionar, siga o roteiro.

### 1. Qual é o valor procurado?
...
### 2. O array tem valores?

Imprima o array com `for`.

### 3. O loop percorre todos os índices válidos?
...
### 4. A comparação usa o valor do array?

Correto:

```java
array[indice] == valorProcurado
```

### 5. A flag começa como false?

```java
boolean encontrou = false;
```

### 6. A flag vira true quando encontra?

Confira dentro do `if`.

### 7. A posição começa como -1?

```java
int posicaoEncontrada = -1;
```

### 8. A posição é atualizada com o índice?

```java
posicaoEncontrada = indice;
```

### 9. O break está adequado?

Use se quer a primeira ocorrência.

Não use se quer contar todas.

### 10. O não encontrado está tratado?

Antes de acessar a posição, verifique:
...
### Teste 1 — Posição inicial errada
...
### Teste 2 — Comparar índice em vez de valor
...
```java
numeros[indice] == valorProcurado
```

### Teste 3 — Esquecer flag
...
Explique por que o resultado é 1.

Depois remova o `break`.

### Teste 5 — Acessar posição não encontrada
...
| Ação | Atalho | Uso |
|---|---|---|
| Reformatar código | `Ctrl + Alt + L` | Organizar busca e if |
| Renomear variável | `Shift + F6` | Melhorar nomes como `valorProcurado` |
| Terminal integrado | `Alt + F12` | Compilar e executar |
| Rodar programa | `Shift + F10` | Executar no IntelliJ |
| Debug | `Shift + F9` | Ver comparação em cada índice |
...
### O que aprendi
Aprendi a procurar valores em arrays usando busca linear, flag `encontrou`, posição encontrada, índice inicial `-1` para não encontrado e `break` para parar ao encontrar a primeira ocorrência.

### O que pratiquei
Criei exemplos de busca simples, busca com posição, busca com Scanner, busca após preencher array, busca e alteração, busca em long, busca por condição, contagem de ocorrências e cenários de produto, pedido, OS, ocorrência, mensageria, estoque e SLA.
...
- não encontrado
- arrays paralelos
- tratamento antes de acessar posição

### Arquivos criados
- `labs/m1/aula-048-busca-array/Main.java`
- `labs/m1/aula-048-busca-array/BuscaComPosicao.java`
- `labs/m1/aula-048-busca-array/ContarOcorrencias.java`
- `labs/m1/aula-048-busca-array/BuscaComScanner.java`
- `labs/m1/aula-048-busca-array/PreencherEBuscarArray.java`
- `labs/m1/aula-048-busca-array/BuscarEAlterarValor.java`
- `labs/m1/aula-048-busca-array/BuscaPagamentoCentavos.java`
- `labs/m1/aula-048-busca-array/PrimeiroProdutoSemEstoque.java`
- `labs/m1/aula-048-busca-array/PrimeiraMensagemComMuitasTentativas.java`
- `labs/m1/aula-048-busca-array/PrimeiroAtendimentoForaSla.java`
- `labs/m1/aula-048-busca-array/BuscarCodigoProduto.java`
- `labs/m1/aula-048-busca-array/BuscarPedidoPorValor.java`
- `labs/m1/aula-048-busca-array/BuscarOsPorQuantidadeAtividades.java`
- `labs/m1/aula-048-busca-array/ContarOsPorQuantidadeAtividades.java`
- `labs/m1/aula-048-busca-array/BuscarCodigoOcorrencia.java`
- `labs/m1/aula-048-busca-array/BuscaEmArrayInformadoUsuario.java`
- `labs/m1/aula-048-busca-array/BuscarProdutoEAjustarEstoque.java`
- `labs/m1/aula-048-busca-array/ListarProdutosSemEstoque.java`
- `labs/m1/aula-048-busca-array/ExisteProdutoSemEstoque.java`
- `labs/m1/aula-048-busca-array/ErroPosicaoInicialZero.java`
- `labs/m1/aula-048-busca-array/ErroCompararIndice.java`
- `labs/m1/aula-048-busca-array/ErroEsquecerFlag.java`
- `labs/m1/aula-048-busca-array/ErroBreakAoContar.java`
- `labs/m1/aula-048-busca-array/ErroAcessarPosicaoNaoEncontrada.java`
...
- inicializar posição encontrada com 0;
- esquecer de mudar a flag `encontrou`;
- comparar índice com valor procurado;
- usar `<= array.length`;
- usar `break` quando queria contar todas as ocorrências;
- não usar `break` quando queria apenas a primeira ocorrência;
- não tratar valor não encontrado;
- usar posição do usuário como índice técnico;
- buscar no array errado;
- comparar dinheiro com `double` em vez de usar centavos.

### Próximo passo
Estudar maior, menor, soma e média em array.
```
...
```text
1. O que é busca em array?
2. O que é busca linear?
3. Para que serve a flag `encontrou`?
4. Por que a posição encontrada costuma começar com `-1`?
5. Qual é a função do `break` em uma busca?
6. O que muda se removermos o `break`?
7. Qual a diferença entre buscar a primeira ocorrência e contar todas?
8. Por que não podemos acessar `array[posicaoEncontrada]` sem verificar se encontrou?
9. Qual a diferença entre índice técnico e posição exibida ao usuário?
10. Quando uma busca por condição é mais adequada do que uma busca por valor exato?
```
