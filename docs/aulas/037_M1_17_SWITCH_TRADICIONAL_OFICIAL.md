...
Estamos no Módulo 1, logo depois de decisões com `if`.
...
Na aula anterior, aprendemos que nem toda decisão deve virar uma escada de `if`.

Vimos que algumas regras ficam melhores com:
...
Agora vamos estudar uma estrutura muito usada quando temos uma variável e queremos tomar decisões com base em valores específicos dela.
...
## Diagnóstico de erro com switch

Quando o `switch` não fizer o esperado, siga o roteiro.

### 1. Qual é o valor avaliado?
...
### 2. O valor é do mesmo tipo dos cases?

`int` com `case 1`.

`String` com `case "PENDENTE"`.

`char` com `case 'A'`.

### 3. Existe `break` em cada case?

Se não, pode haver fall-through.

### 4. Existe `default`?

Se não, valor inesperado pode passar sem mensagem.

### 5. A String tem espaços?

Use `trim()`.

### 6. A String tem caixa diferente?

Use `toUpperCase()` ou `equalsIgnoreCase` em `if`, conforme regra.

No `switch`, padronize antes.

### 7. A variável pode ser null?

Trate antes do switch.

### 8. Você está tentando usar intervalo?

Use `if`.

### 9. Os cases estão grandes demais?

Talvez precise dividir responsabilidades.

### 10. Testou cada case?

Teste todos os valores, inclusive inválido.
...
### Teste 1 — Fall-through acidental
...
Execute e observe que nada aparece.

Depois adicione `default`.

### Teste 3 — String sem padronizar
...
```java
String status = "aprovado".toUpperCase();
```

### Teste 4 — Null no switch
...
Depois trate com `if (status == null)` antes.
...
| Ação | Atalho | Uso |
|---|---|---|
| Reformatar código | `Ctrl + Alt + L` | Organizar blocos do switch |
...
### O que aprendi
Aprendi a usar `switch` tradicional com `case`, `break` e `default`, entendendo menus, status e o risco de `fall-through`.

### O que pratiquei
Criei exemplos com menu, status de pedido, status de OS, tipo de operação, mensageria, prioridade, código de ocorrência e entrada com Scanner.
...
- char em switch
- int em switch
- tratamento antes do switch
- risco de null
- comparação com if/else if

### Arquivos criados
- `labs/m1/aula-037-switch-tradicional/Main.java`
- `labs/m1/aula-037-switch-tradicional/MenuSimples.java`
- `labs/m1/aula-037-switch-tradicional/MenuConsole.java`
- `labs/m1/aula-037-switch-tradicional/SwitchStatusPedido.java`
- `labs/m1/aula-037-switch-tradicional/SwitchPrioridade.java`
- `labs/m1/aula-037-switch-tradicional/SwitchPedido.java`
- `labs/m1/aula-037-switch-tradicional/SwitchOrdemServico.java`
- `labs/m1/aula-037-switch-tradicional/SwitchOperacao.java`
- `labs/m1/aula-037-switch-tradicional/SwitchMensageria.java`
- `labs/m1/aula-037-switch-tradicional/SwitchPrioridadeAtendimento.java`
- `labs/m1/aula-037-switch-tradicional/SwitchOcorrencia.java`
- `labs/m1/aula-037-switch-tradicional/SwitchStatusConsole.java`
- `labs/m1/aula-037-switch-tradicional/MenuOperacoesConsole.java`
- `labs/m1/aula-037-switch-tradicional/FallThroughAcidental.java`
- `labs/m1/aula-037-switch-tradicional/FallThroughIntencional.java`
- `labs/m1/aula-037-switch-tradicional/SwitchNull.java`
...
- esquecer `break`;
- achar que `default` evita fall-through;
- esquecer `default`;
- usar switch para intervalo;
- usar String sem padronizar entrada;
- usar switch com String possivelmente null;
- colocar muita responsabilidade dentro de um case;
- duplicar lógica em vários cases;
- usar case com tipo incompatível;
- tentar colocar condição booleana em case tradicional.

### Próximo passo
Estudar switch moderno e expressões.
```
...
2. Qual é a função do `case`?
3. Qual é a função do `break`?
4. Qual é a função do `default`?
5. O que é fall-through?
6. Por que esquecer `break` pode ser perigoso?
7. Quando o switch pode ser melhor que if/else if?
8. Quando if é melhor que switch?
9. Por que é importante padronizar String antes do switch?
10. Por que `default` não trata `null` em switch com String?
```
