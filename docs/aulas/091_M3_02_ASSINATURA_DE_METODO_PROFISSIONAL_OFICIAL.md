...
debug funciona;
renomear método e parâmetro pela IDE está acessível.
```

A prática desta aula será feita em:

```text
labs/m3/aula-091-assinatura-de-metodo-profissional
```
...
Na aula anterior, aprendemos a sair de um `main` gigante para métodos menores.

Agora vamos melhorar a qualidade desses métodos.

Não basta extrair método.
...
### 1. Consigo entender a intenção só pela assinatura?

Se não, renomeie.

### 2. O verbo combina com o retorno?

`calcular` deve retornar.

`imprimir` pode ser void.

### 3. Os parâmetros representam o domínio?

`precoUnitario` é melhor que `valor`.

### 4. O tipo está correto?

Dinheiro como `BigDecimal`.

Data como `LocalDate`.

Opção fixa como `enum`.

### 5. O boolean poderia ser enum?

Se a chamada não fica clara, sim.

### 6. Há muitos parâmetros?

Considere record/classe futuramente.

### 7. O método tem responsabilidade demais?

Talvez precise dividir.

### 8. O nome é grande demais?

Nome descritivo é bom, mas nome enorme pode indicar método fazendo demais.

### 9. Há ambiguidade?

Elimine.

### 10. O código que chama ficou legível?

Assinatura boa melhora chamada.
...
### Teste 1 — trocar ordem de parâmetros

Crie método:

```java
calcularDiasEntre(LocalDate fim, LocalDate inicio)
```

Chame sem perceber a inversão.

Explique o risco.

---

### Teste 2 — boolean misterioso
...
Depois troque para enum.

Compare leitura.

---

### Teste 3 — void para cálculo

Crie método que calcula e imprime.

Depois refatore para retornar.

Compare reuso.

---

### Teste 4 — parâmetros a, b, c

Renomeie para nomes claros.

Compare debug.

---

### Teste 5 — método genérico
...
```markdown
# Aula 091 — Assinatura de método profissional

## Objetivo

Aprender a desenhar assinaturas de método com nomes claros, parâmetros bem definidos, tipos adequados, retorno coerente e intenção explícita.

## Conceitos

- Assinatura de método é contrato.
...
- Ordem dos parâmetros deve ser natural.
- Boolean em parâmetro pode ser ambíguo.
- Muitos parâmetros podem indicar conceito faltando.
- `void` faz sentido para ação, não para cálculo.
- Retorno deve representar resultado.
- Assinatura boa melhora leitura, debug e manutenção.
...
| Renomear | `Shift + F6` | Melhorar método/parâmetro |
| Alterar assinatura | ação Change Signature | Reordenar parâmetros |
| Buscar ação | `Ctrl + Shift + A` | Encontrar refatorações |
| Extrair método | ação Extract Method | Criar método com assinatura |
| Debug | `Shift + F9` | Observar parâmetros e retornos |
| Step Into | `F7` em muitos keymaps | Entrar no método |
| Step Over | `F8` em muitos keymaps | Avançar chamada |
| Quick Documentation | ação da IDE | Conferir assinatura |
| Compilar | `javac Arquivo.java` | Validar código |
| Executar | `java Classe` | Validar comportamento |
...
### O que aprendi
Aprendi que assinatura de método é um contrato e que nome, parâmetros, retorno, ordem, tipo e intenção precisam comunicar claramente o comportamento esperado.

### O que pratiquei
Comparei assinaturas ruins e boas, refatorei nomes genéricos, substituí boolean misterioso por enum, separei cálculo de impressão, melhorei parâmetros e apliquei assinaturas profissionais em pedido, cliente, produto, pagamento, OS, mensageria e auditoria.
...
### Arquivos criados
- `labs/m3/aula-091-assinatura-de-metodo-profissional/AssinaturaRuim.java`
- `labs/m3/aula-091-assinatura-de-metodo-profissional/AssinaturaBoa.java`
- `labs/m3/aula-091-assinatura-de-metodo-profissional/PedidoAssinaturaProfissional.java`
- `labs/m3/aula-091-assinatura-de-metodo-profissional/ClienteAssinaturaProfissional.java`
- `labs/m3/aula-091-assinatura-de-metodo-profissional/ProdutoAssinaturaProfissional.java`
- `labs/m3/aula-091-assinatura-de-metodo-profissional/PagamentoAssinaturaProfissional.java`
- `labs/m3/aula-091-assinatura-de-metodo-profissional/OrdemServicoAssinaturaProfissional.java`
- `labs/m3/aula-091-assinatura-de-metodo-profissional/MensageriaAssinaturaProfissional.java`
- `labs/m3/aula-091-assinatura-de-metodo-profissional/AuditoriaAssinaturaProfissional.java`
- `labs/m3/aula-091-assinatura-de-metodo-profissional/README.md`
...
- `void` para cálculo;
- retorno genérico;
- parâmetros em ordem confusa;
- parâmetros demais sem perceber;
- nome que promete uma coisa e corpo faz outra;
- assinatura que obriga abrir o método para entender;
- refatorar assinatura sem rodar o código.

### Próximo passo
Estudar coesão em métodos.
```
...
```text
1. O que é assinatura de método?
2. Por que assinatura é contrato?
3. Quais elementos compõem uma assinatura profissional?
4. Por que nome do método importa?
5. Qual problema de nomes genéricos?
6. Como o verbo do método influencia o retorno?
7. Quando um método deve retornar valor?
8. Quando faz sentido usar void?
9. Por que parâmetros a, b, c são ruins?
10. Como escolher tipo de parâmetro?
11. Por que BigDecimal é melhor para dinheiro?
12. Por que LocalDate é melhor para data sem horário?
13. O que é boolean misterioso?
14. Como enum pode melhorar uma assinatura?
15. Por que ordem dos parâmetros importa?
16. O que muitos parâmetros podem indicar?
17. Por que retorno Object geralmente é ruim?
18. Como assinatura melhora debug?
19. Como assinatura melhora manutenção?
20. Qual assinatura você melhorou nesta aula e por quê?
```
