...
Na aula anterior, estudamos `var` com critério.

Agora vamos estudar um recurso muito usado em bibliotecas, utilitários e métodos de conveniência:

```text
varargs.
```

Varargs permite criar métodos que recebem uma quantidade variável de argumentos.
...
Mas o uso profissional exige cuidado com:
...
## Diagnóstico de varargs

Quando revisar um método com varargs, pergunte:

### 1. Os argumentos têm a mesma natureza?

Se não, talvez varargs seja ruim.

### 2. Zero argumentos faz sentido?

Se não, valide.

### 3. Null foi tratado?

Valide array e elementos.

### 4. O varargs está no final?

Precisa estar.

### 5. Há sobrecarga parecida?

Verifique ambiguidade.

### 6. Object... é realmente necessário?

Se não, use tipo forte.

### 7. Um record/request seria mais claro?

Se sim, evite varargs.

### 8. O método ficou legível?

A chamada precisa ser clara.

### 9. Há validação de negócio?

Não aceite qualquer coisa.

### 10. Use debug

Veja o array recebido.
...
### Teste 1 — zero argumentos sem validação
...
### Teste 2 — varargs fora do final

```java
public class Main {
    public static void metodo(String... textos, int numero) {
    }
}
```
...
### Teste 3 — dois varargs

```java
public class Main {
    public static void metodo(String... textos, int... numeros) {
    }
}
```
...
### Teste 5 — Object... bagunçado

```java
public class Main {
    public static void main(String[] args) {
        registrar("Ana", 10, true, null);
    }
...
Explique por que compila, mas pode ser modelagem ruim.
...
Entender métodos com quantidade variável de argumentos em Java, sabendo como funcionam por dentro, quando usar, quando evitar e quais cuidados tomar com arrays, null, sobrecarga e ambiguidade.

## Conceitos

- Varargs permite quantidade variável de argumentos.
- A sintaxe usa reticências: `String... textos`.
- Dentro do método, varargs é tratado como array.
- Varargs pode receber zero argumentos.
- Varargs pode receber array explícito.
- Varargs deve ser o último parâmetro.
- Só pode haver um varargs por método.
- É preciso validar array null e elementos null quando necessário.
- Varargs combina bem com métodos utilitários.
- Varargs é ruim quando argumentos têm naturezas diferentes.
- `Object...` deve ser usado com cuidado.
- Sobrecarga com varargs pode gerar confusão.
- Quando os dados têm significado próprio, record/request pode ser melhor.
...
| Debug | `Shift + F9` | Ver array interno do varargs |
| Run | `Shift + F10` | Executar normal |
| Step Over | `F8` em muitos keymaps | Avançar loop |
| Variables | janela Debug | Ver `length` e elementos |
| Evaluate Expression | `Alt + F8` em muitos keymaps | Testar chamadas diferentes |
...
### O que aprendi
Aprendi que varargs permite criar métodos com quantidade variável de argumentos e que, por dentro, esse parâmetro é tratado como array. Também aprendi que varargs deve ser o último parâmetro, pode receber zero argumentos e exige cuidado com null, sobrecarga e legibilidade.

### O que pratiquei
Criei exemplos com impressão de textos, soma de números, média, array explícito, separador, BigDecimal, null, sobrecarga, Object varargs e aplicações em cliente, produto, pedido, pagamento, OS, mensageria e auditoria.
...
### Arquivos criados
- `labs/m2/aula-079-varargs/Main.java`
- `labs/m2/aula-079-varargs/VarargsZeroArgumentos.java`
- `labs/m2/aula-079-varargs/VarargsArrayExplicito.java`
- `labs/m2/aula-079-varargs/SomarVarargs.java`
- `labs/m2/aula-079-varargs/MediaVarargs.java`
- `labs/m2/aula-079-varargs/VarargsUltimoParametro.java`
- `labs/m2/aula-079-varargs/ErroVarargsNaoUltimo.java`
- `labs/m2/aula-079-varargs/ErroDoisVarargs.java`
- `labs/m2/aula-079-varargs/ConcatenarTextos.java`
- `labs/m2/aula-079-varargs/JuntarComSeparador.java`
- `labs/m2/aula-079-varargs/SomarBigDecimalVarargs.java`
- `labs/m2/aula-079-varargs/VarargsNullExplicito.java`
- `labs/m2/aula-079-varargs/SobrecargaVarargs.java`
- `labs/m2/aula-079-varargs/ErroObjectVarargs.java`
- `labs/m2/aula-079-varargs/ClienteVarargs.java`
- `labs/m2/aula-079-varargs/ProdutoVarargs.java`
- `labs/m2/aula-079-varargs/PedidoVarargs.java`
- `labs/m2/aula-079-varargs/PagamentoVarargs.java`
- `labs/m2/aula-079-varargs/OrdemServicoVarargs.java`
- `labs/m2/aula-079-varargs/MensageriaVarargs.java`
- `labs/m2/aula-079-varargs/AuditoriaVarargs.java`
- `labs/m2/aula-079-varargs/DebugVarargs.java`
- `labs/m2/aula-079-varargs/ErroMediaSemArgumentos.java`
- `labs/m2/aula-079-varargs/ErroElementoNull.java`
- `labs/m2/aula-079-varargs/ErroVarargsNaturezasDiferentes.java`
- `labs/m2/aula-079-varargs/README.md`
...
- achar que varargs não é array;
- esquecer que zero argumentos é permitido;
- não validar null;
- colocar varargs antes de outro parâmetro;
- tentar ter dois varargs no mesmo método;
- usar `Object...` sem necessidade;
- criar sobrecargas ambíguas;
- usar varargs para argumentos de naturezas diferentes;
- ignorar custo de array em caminho crítico;
- usar varargs onde um record/request seria melhor.

### Próximo passo
Estudar annotations básicas.
```
...
```text
1. O que é varargs?
2. Qual é a sintaxe de varargs?
3. O que significa `String... textos`?
4. Como varargs é tratado dentro do método?
5. Varargs permite zero argumentos?
6. O que acontece se chamar um método varargs sem argumentos?
7. Varargs pode receber array explícito?
8. Varargs precisa ser o último parâmetro?
9. Pode haver dois varargs no mesmo método?
10. Por que validar length pode ser necessário?
11. Qual o risco de elemento null?
12. Qual o risco de array null explícito?
13. O que é sobrecarga com varargs?
14. Por que sobrecarga com varargs pode confundir?
15. Quando usar varargs?
16. Quando evitar varargs?
17. Por que Object... exige cuidado?
18. Por que varargs cria array?
19. Quando um record/request é melhor que varargs?
20. Como varargs pode ajudar em utilitários?
```
