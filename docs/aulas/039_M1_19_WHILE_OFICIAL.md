...
Estamos no Módulo 1, entrando em repetição.
...
Até agora, o programa conseguia:
...
combinar condições;
decidir com if;
escolher caminhos com switch.
```

Agora o programa vai aprender a repetir.
...
O `while` é a primeira estrutura de repetição que vamos estudar.
...
## Diagnóstico de erro com while

Quando um `while` não funcionar, siga o roteiro.

### 1. A condição inicial é verdadeira?

Se não for, o loop não executa.

### 2. A condição muda dentro do loop?

Se não muda, pode virar loop infinito.

### 3. A variável muda na direção certa?

Se a condição é `contador <= 5`, o contador precisa aumentar.

### 4. O limite está correto?

Verifique `<`, `<=`, `>`, `>=`.

### 5. Existe valor sentinela?

Se sim, ele está sendo lido corretamente?

### 6. A entrada é atualizada dentro do loop?

Em loops com Scanner, precisa ler novamente.

### 7. Contador e acumulador estão separados?

Verifique nomes e funções.

### 8. Alguma variável foi declarada no escopo errado?

Se precisa usar depois, declare antes.

### 9. O loop pode executar zero vezes?

Teste esse cenário.

### 10. Use debug ou prints

Imprima a variável de controle:
...
### Teste 1 — Loop infinito por falta de incremento
...
Se travar, interrompa o programa.
...
### Teste 3 — Condição inicial falsa
...
Observe que o bloco não executa.
...
### Teste 5 — Entrada não atualizada
...
Depois corrija lendo novamente dentro do loop.
...
| Ação | Atalho | Uso |
|---|---|---|
| Reformatar código | `Ctrl + Alt + L` | Organizar blocos do while |
| Renomear variável | `Shift + F6` | Melhorar nome de contador |
...
### O que aprendi
Aprendi que `while` repete um bloco enquanto uma condição booleana for verdadeira, e que ele testa a condição antes de executar.

### O que pratiquei
Criei exemplos com contador crescente, contador decrescente, acumulador, menu, valor sentinela, validação de entrada, processamento de lote, tentativas, estoque, paginação, mensageria e auditoria.
...
### Arquivos criados
- `labs/m1/aula-039-while/Main.java`
- `labs/m1/aula-039-while/WhileCrescente.java`
- `labs/m1/aula-039-while/WhileDecrescente.java`
- `labs/m1/aula-039-while/AcumuladorWhile.java`
- `labs/m1/aula-039-while/LeituraEnquantoPositivo.java`
- `labs/m1/aula-039-while/MenuWhile.java`
- `labs/m1/aula-039-while/ProcessamentoLoteWhile.java`
- `labs/m1/aula-039-while/ProcessamentoLoteComErro.java`
- `labs/m1/aula-039-while/TentativasWhile.java`
- `labs/m1/aula-039-while/TentativaSenhaConsole.java`
- `labs/m1/aula-039-while/BaixaEstoqueWhile.java`
- `labs/m1/aula-039-while/PaginacaoWhile.java`
- `labs/m1/aula-039-while/MensageriaWhile.java`
- `labs/m1/aula-039-while/AuditoriaWhile.java`
- `labs/m1/aula-039-while/ValidacaoEntradaWhile.java`
- `labs/m1/aula-039-while/WhileBreak.java`
- `labs/m1/aula-039-while/WhileContinue.java`
...
- não perceber que a condição inicial pode ser falsa;
- usar `<` quando deveria usar `<=`;
- usar `<=` quando deveria usar `<`;
- não atualizar entrada dentro do loop;
- criar `while (true)` sem saída clara;
- confundir contador com acumulador;
- declarar variável dentro do while e tentar usar fora;
- não testar cenário de zero iterações.
...
```text
1. Para que serve o `while`?
2. O que significa dizer que o while testa antes de executar?
3. O while pode executar zero vezes? Por quê?
4. O que é loop infinito?
5. Quais são as três partes comuns de um while com contador?
6. O que é valor sentinela?
7. Por que a variável da condição precisa mudar dentro do loop?
8. Qual a diferença entre contador e acumulador dentro de um while?
9. Quando `break` interrompe o loop?
10. Por que é importante testar o cenário de zero iterações?
```
