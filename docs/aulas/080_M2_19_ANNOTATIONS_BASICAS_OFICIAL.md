...
- `M2.19.01` — Annotations básicas — Conceito profundo e quando usar.
- `M2.19.02` — Annotations básicas — Implementação guiada com código realista.
- `M2.19.03` — Annotations básicas — Refatoração, melhoria e leitura crítica.
- `M2.19.04` — Annotations básicas — Exercício solo, perguntas e critério de aprovação.

Nada dessas sessões foi removido.

O conteúdo foi integrado em uma única aula mentorada para ensinar `@Override`, `@Deprecated`, `@SuppressWarnings` e leitura básica de metadados, incluindo motivação, alternativas, trade-offs, armadilhas, implementação guiada, refatoração, leitura crítica e aplicação em cenários de cliente, produto, pedido, pagamento, OS, auditoria e mensageria.
...
Na aula anterior, estudamos varargs.

Agora vamos estudar um recurso que aparece em praticamente todo projeto Java moderno:

```text
annotations.
```

Você verá annotations em Java puro:
...
E também verá muito em frameworks:
...
Nesta aula, o foco ainda não é Spring, JPA ou testes automatizados.
...
por que ela existe;
como o compilador usa;
como frameworks podem ler;
como usar as básicas com responsabilidade.
```
...
```text
Aulas oficiais concluídas: 80
Aulas oficiais restantes: 458
```

Contando o arquivo de abertura `000`, teremos:

```text
81 arquivos gerados no total.
```

Ainda estamos no começo do Módulo 2, consolidando Java Core antes de avançar para reflection conceitual, classes, encapsulamento, coleções, exceções, Maven, banco de dados, Spring Boot e arquitetura backend.
...
## Diagnóstico de annotations

Quando revisar código com annotation, pergunte:

### 1. Quem usa esta annotation?
...
### 2. Ela tem efeito real?

Se não tem, talvez esteja sobrando.

### 3. O escopo está correto?

Classe inteira ou só método?

### 4. O warning suprimido é específico?

Evite `"all"`.

### 5. O deprecated tem alternativa?

Precisa indicar migração.

### 6. A annotation precisa ser lida em runtime?

Se sim, confira `@Retention(RUNTIME)`.

### 7. A annotation está no alvo certo?

Confira `@Target`.

### 8. Existe regra escondida demais?

Talvez método explícito seja melhor.

### 9. O nome da annotation é claro?

Nome deve expressar intenção.

### 10. Use debug/leitura

Se for runtime, teste `getAnnotation`.
...
### Teste 1 — @Override com nome errado
...
### Teste 2 — value deprecated

Crie um método deprecated e chame no `main`.

Observe o aviso da IDE/compilador.

---

### Teste 3 — suppress amplo
...
### Teste 4 — annotation sem RUNTIME
...
Explique por que pode retornar `null`.

---

### Teste 5 — annotation sem leitor

Crie uma annotation e use em uma classe.

Não escreva nenhum código para ler.

Explique por que nada acontece sozinho.
...
Entender annotations básicas em Java, incluindo `@Override`, `@Deprecated`, `@SuppressWarnings` e leitura inicial de metadados com annotations customizadas.

## Conceitos

- Annotation é metadado no código.
- Annotation começa com `@`.
- Annotation não é comentário.
- `@Override` valida sobrescrita/implementação de método.
- `@Deprecated` marca API antiga ou não recomendada.
- `@SuppressWarnings` suprime avisos específicos.
- Suprimir warning não corrige problema.
- Escopo menor é melhor ao suprimir warning.
- Annotation customizada usa `@interface`.
- `@Retention` define disponibilidade da annotation.
- `@Target` define onde a annotation pode ser usada.
- Para ler em runtime, use `RetentionPolicy.RUNTIME`.
- Annotation não executa regra sozinha.
- Algum código, framework ou ferramenta precisa interpretar a annotation.
...
| Debug | `Shift + F9` | Ver leitura de annotation |
| Run | `Shift + F10` | Executar normal |
| Step Over | `F8` em muitos keymaps | Avançar getAnnotation |
| Variables | janela Debug | Ver annotation lida |
| Evaluate Expression | `Alt + F8` em muitos keymaps | Testar `getAnnotation` |
| Reformatar código | `Ctrl + Alt + L` | Organizar |
| Renomear | `Shift + F6` | Ajustar nomes |
| Extrair método | `Ctrl + Alt + M` em muitos keymaps | Separar leitura/validação |
...
### O que aprendi
Aprendi que annotations são metadados no código e que podem ser usadas pelo compilador, IDE, ferramentas, frameworks ou runtime. Também aprendi a usar `@Override`, `@Deprecated`, `@SuppressWarnings` e a criar uma annotation simples com leitura de metadado.

### O que pratiquei
Criei exemplos com `@Override`, erro de sobrescrita, interface, método deprecated, suppress warnings, annotation customizada, `@Retention`, `@Target`, leitura com `getAnnotation` e aplicações em cliente, produto, pedido, pagamento, OS, mensageria e auditoria.
...
### Arquivos criados
- `labs/m2/aula-080-annotations-basicas/Main.java`
- `labs/m2/aula-080-annotations-basicas/ErroOverride.java`
- `labs/m2/aula-080-annotations-basicas/OverrideInterface.java`
- `labs/m2/aula-080-annotations-basicas/DeprecatedBasico.java`
- `labs/m2/aula-080-annotations-basicas/DeprecatedComDetalhe.java`
- `labs/m2/aula-080-annotations-basicas/DeprecatedComAlternativa.java`
- `labs/m2/aula-080-annotations-basicas/SuppressWarningsDeprecation.java`
- `labs/m2/aula-080-annotations-basicas/SuppressWarningsUnchecked.java`
- `labs/m2/aula-080-annotations-basicas/Tabela.java`
- `labs/m2/aula-080-annotations-basicas/ClienteTabela.java`
- `labs/m2/aula-080-annotations-basicas/LeituraAnnotation.java`
- `labs/m2/aula-080-annotations-basicas/CampoObrigatorio.java`
- `labs/m2/aula-080-annotations-basicas/ClienteComCampoObrigatorio.java`
- `labs/m2/aula-080-annotations-basicas/LeituraCampoObrigatorio.java`
- `labs/m2/aula-080-annotations-basicas/ClienteAnnotations.java`
- `labs/m2/aula-080-annotations-basicas/ProdutoAnnotations.java`
- `labs/m2/aula-080-annotations-basicas/PedidoAnnotations.java`
- `labs/m2/aula-080-annotations-basicas/PagamentoAnnotations.java`
- `labs/m2/aula-080-annotations-basicas/OrdemServicoAnnotations.java`
- `labs/m2/aula-080-annotations-basicas/CanalMensagem.java`
- `labs/m2/aula-080-annotations-basicas/MensageriaAnnotations.java`
- `labs/m2/aula-080-annotations-basicas/LeituraCanalMensagem.java`
- `labs/m2/aula-080-annotations-basicas/AuditoriaAnnotations.java`
- `labs/m2/aula-080-annotations-basicas/DebugAnnotation.java`
- `labs/m2/aula-080-annotations-basicas/ErroAnnotationSemRuntime.java`
- `labs/m2/aula-080-annotations-basicas/ErroSuppressWarningsAll.java`
- `labs/m2/aula-080-annotations-basicas/ErroDeprecatedSemAlternativa.java`
- `labs/m2/aula-080-annotations-basicas/README.md`
...
- usar `@SuppressWarnings("all")`;
- deprecar sem indicar alternativa;
- ignorar warning de deprecated;
- criar annotation sem retention correto;
- criar annotation sem target adequado;
- achar que annotation customizada executa regra sozinha;
- colocar regra demais em annotation.

### Próximo passo
Estudar reflection conceitual.
```
...
```text
1. O que é annotation?
2. Annotation é comentário?
3. Para que serve @Override?
4. Que erro @Override ajuda a detectar?
5. Para que serve @Deprecated?
6. Deprecated significa removido?
7. Para que serve since em @Deprecated?
8. Para que serve forRemoval em @Deprecated?
9. Para que serve @SuppressWarnings?
10. Por que @SuppressWarnings pode ser perigoso?
11. Por que evitar @SuppressWarnings("all")?
12. Qual é a vantagem de suprimir warning no menor escopo?
13. Como criar uma annotation customizada?
14. Para que serve @Retention?
15. Para que serve @Target?
16. O que significa RetentionPolicy.RUNTIME?
17. O que acontece se uma annotation não estiver disponível em runtime?
18. Annotation customizada executa regra sozinha?
19. Quem pode ler annotations?
20. Por que annotations são importantes em frameworks?
```
