...
```text
campos de referência começam como null;
arrays de referência começam com posições null;
variáveis locais precisam ser inicializadas;
default técnico não é regra de negócio;
objeto criado pode estar incompleto.
```

Agora vamos aprofundar o risco mais famoso ligado a referências em Java:

```text
NullPointerException.
```

Esse erro acontece quando tentamos usar uma referência que está `null`.
...
A variável `nome` existe.

Mas ela não aponta para nenhum objeto `String`.

Então não existe objeto onde chamar `length()`.
...
## Diagnóstico de NullPointerException

Quando aparecer NPE, siga o roteiro.

### 1. Qual linha quebrou?
...
### 3. A variável foi inicializada?

Procure onde recebeu valor.

### 4. É campo com default null?

Campo `String` não inicializado começa null.

### 5. É posição de array?

Verifique se a posição recebeu `new`.

### 6. É retorno de método?

Verifique se o método pode retornar null.

### 7. É parâmetro recebido?

Verifique quem chamou e o que enviou.

### 8. A ausência é aceitável?

Se sim, trate com mensagem ou Optional.

Se não, lance exceção clara.

### 9. Existe equals inseguro?
...
Pare antes da linha e veja qual referência está null.
...
### Teste 2 — Objeto existe, campo null
...
Explique a diferença entre `cliente` e `cliente.nome`.

---

### Teste 3 — Array com posição null
...
Explique por que `clientes[1]` está null.

---

### Teste 4 — equals inseguro
...
Explique por que Optional não elimina necessidade de pensar.

Corrija com:

```java
nome.orElse("NAO_INFORMADO")
```
...
```markdown
# Aula 066 — Null e NullPointerException

## Objetivo

Entender o que é `null`, por que `NullPointerException` acontece, como diagnosticar, como validar referências e como usar `Optional` no momento certo para retornos opcionais.

## Conceitos

- `null` significa ausência de referência.
- `NullPointerException` acontece ao usar `null` como objeto.
- Campo de referência não inicializado começa `null`.
- Array de objetos começa com posições `null`.
- Método pode retornar `null` quando o contrato permitir.
- Quem recebe retorno possivelmente `null` deve validar.
- A ordem da validação importa.
- `"VALOR".equals(variavel)` evita NPE em comparação.
- `Objects.requireNonNull` ajuda a falhar cedo.
- `Optional` é útil principalmente em retorno de busca.
- `Optional.get()` sem verificar presença é perigoso.
- Stack trace mostra onde o erro aconteceu.
...
| Debug | `Shift + F9` | Ver qual referência está null |
| Run | `Shift + F10` | Executar normal |
| Step Into | `F7` em muitos keymaps | Entrar em método que recebe null |
| Step Over | `F8` em muitos keymaps | Avançar validações |
| Variables | janela Debug | Inspecionar campos null |
...
### O que aprendi
Aprendi que `null` representa ausência de referência e que `NullPointerException` acontece quando tento usar uma referência null como se fosse objeto. Também aprendi a validar null, ler stack trace, usar equals seguro e aplicar Optional em retornos de busca.

### O que pratiquei
Criei exemplos com String null, campo interno null, array de objetos com posições null, método retornando null, Optional, Objects.requireNonNull, equals seguro e validações em pedido, produto, pagamento, OS, mensageria e auditoria.
...
### Arquivos criados
- `labs/m2/aula-066-null-nullpointerexception/Main.java`
- `labs/m2/aula-066-null-nullpointerexception/ValidacaoNullSimples.java`
- `labs/m2/aula-066-null-nullpointerexception/TextoPreenchido.java`
- `labs/m2/aula-066-null-nullpointerexception/ErroOrdemValidacao.java`
- `labs/m2/aula-066-null-nullpointerexception/CampoNull.java`
- `labs/m2/aula-066-null-nullpointerexception/CampoNullCorrigido.java`
- `labs/m2/aula-066-null-nullpointerexception/CriarClienteSeguro.java`
- `labs/m2/aula-066-null-nullpointerexception/ClienteObrigatorio.java`
- `labs/m2/aula-066-null-nullpointerexception/RequireNonNullExemplo.java`
- `labs/m2/aula-066-null-nullpointerexception/RequireNonNullTexto.java`
- `labs/m2/aula-066-null-nullpointerexception/EqualsSeguro.java`
- `labs/m2/aula-066-null-nullpointerexception/ErroEqualsInseguro.java`
- `labs/m2/aula-066-null-nullpointerexception/ArrayObjetoNull.java`
- `labs/m2/aula-066-null-nullpointerexception/ErroArrayObjetoNull.java`
- `labs/m2/aula-066-null-nullpointerexception/BuscaRetornaNull.java`
- `labs/m2/aula-066-null-nullpointerexception/ErroBuscaRetornaNull.java`
- `labs/m2/aula-066-null-nullpointerexception/BuscaComOptional.java`
- `labs/m2/aula-066-null-nullpointerexception/OptionalOrElse.java`
- `labs/m2/aula-066-null-nullpointerexception/OptionalOrElseThrow.java`
- `labs/m2/aula-066-null-nullpointerexception/PedidoNullSeguro.java`
- `labs/m2/aula-066-null-nullpointerexception/ProdutoNullSeguro.java`
- `labs/m2/aula-066-null-nullpointerexception/PagamentoNullSeguro.java`
- `labs/m2/aula-066-null-nullpointerexception/OrdemServicoNullSeguro.java`
- `labs/m2/aula-066-null-nullpointerexception/MensageriaNullSeguro.java`
- `labs/m2/aula-066-null-nullpointerexception/AuditoriaNullSeguro.java`
- `labs/m2/aula-066-null-nullpointerexception/StackTraceNpe.java`
- `labs/m2/aula-066-null-nullpointerexception/DebugNullPointer.java`
- `labs/m2/aula-066-null-nullpointerexception/ErroStringNull.java`
- `labs/m2/aula-066-null-nullpointerexception/ErroCampoInternoNull.java`
- `labs/m2/aula-066-null-nullpointerexception/ErroOptionalGet.java`
- `labs/m2/aula-066-null-nullpointerexception/README.md`
...
- chamar método em referência null;
- acessar campo de objeto null;
- acessar campo null dentro de objeto existente;
- acessar posição null de array de objetos;
- usar equals do lado possivelmente null;
- ignorar método que retorna null;
- usar Optional.get sem verificar presença;
- usar Optional em campo sem necessidade;
- trocar NPE por valor falso sem regra;
- não ler stack trace.

### Próximo passo
Estudar String pool e imutabilidade de String.
```
...
```text
1. O que significa null?
2. O que é NullPointerException?
3. Qual a diferença entre null, texto vazio e texto em branco?
4. Por que campo String não inicializado começa null?
5. Por que array de Cliente começa com posições null?
6. Qual é o risco de cliente.nome.length()?
7. Qual é a ordem correta para validar texto preenchido?
8. Por que "APROVADO".equals(status) é mais seguro?
9. O que é contrato de método?
10. Quando um método pode retornar null?
11. Qual o risco de ignorar retorno null?
12. Para que serve Objects.requireNonNull?
13. Quando Optional é adequado?
14. Qual o risco de Optional.get?
15. Como ler um stack trace de NPE?
```
