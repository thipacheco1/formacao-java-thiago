...
debug funciona;
atalho de refatoração está acessível pela IDE ou pelo menu.
```

A prática desta aula será feita em:

```text
labs/m3/aula-090-de-main-gigante-para-metodos-pequenos
```
...
Estamos iniciando o Módulo 3:

```text
Métodos, organização procedural e projetos console.
```

Até aqui, fechamos o Módulo 2 com Java Core profundo.
...
Antes de entrar com força em orientação a objetos, precisamos melhorar a organização procedural.

Isso significa:

```text
aprender a quebrar um fluxo grande em métodos menores;
dar nomes bons;
separar responsabilidades;
reduzir duplicação;
melhorar leitura;
preparar o raciocínio para classes e objetos.
```

Muita gente tenta aprender orientação a objetos antes de saber organizar um `main`.
...
Quando extrair método, pergunte:

### 1. O método tem nome claro?

O nome explica intenção?

### 2. O método tem uma responsabilidade principal?

Ou faz muitas coisas?

### 3. Os parâmetros fazem sentido?

Ou estão em excesso?

### 4. O retorno faz sentido?

Cálculo retorna valor?

### 5. O método altera algo inesperado?

Há efeito colateral escondido?

### 6. O main ficou mais legível?

Ou só deslocamos bagunça?

### 7. O comportamento foi preservado?

A saída antes e depois é a mesma?

### 8. A duplicação diminuiu?

Ou aumentou?

### 9. O método pode ser testado isoladamente?

Conseguimos chamar com entradas diferentes?

### 10. O código compila e roda?
...
### Teste 3 — cliente em branco

```java
String cliente = normalizarNome("   ");
```
...
### Teste 4 — total com desconto

Use preço e quantidade que gerem total maior ou igual a 300.

Verifique desconto.

---

### Teste 5 — remover return após validação
...
após validação inválida e veja comportamento ruim.

Explique por que o fluxo precisa parar.
...
```markdown
# Aula 090 — De main gigante para métodos pequenos

## Objetivo

Aprender a identificar sinais de `main` inchado e refatorar o fluxo em métodos pequenos, com nomes claros, parâmetros adequados, retorno correto e responsabilidades separadas.
...
- Método pequeno não é método artificial.
- Comentários podem denunciar métodos ausentes.
- Cálculo deve retornar valor.
- Impressão pode ser `void`.
- Validação pode retornar `boolean` em casos simples.
- Refatoração deve ser incremental.
- Depois de refatorar, compile e execute.
- Código refatorado deve preservar comportamento.
...
| Extrair método | ação Extract Method | Refatorar blocos |
| Buscar ação | `Ctrl + Shift + A` | Encontrar refatoração |
...
### O que aprendi
Aprendi a identificar sinais de `main` inchado e a extrair métodos pequenos com nomes claros, parâmetros adequados, retorno correto e responsabilidade principal.

### O que pratiquei
Refatorei exemplos de pedido, cliente, produto, pagamento, ordem de serviço, mensageria e auditoria, separando normalização, validação, cálculo, montagem de mensagem e impressão.
...
### Arquivos criados
- `labs/m3/aula-090-de-main-gigante-para-metodos-pequenos/MainGigante.java`
- `labs/m3/aula-090-de-main-gigante-para-metodos-pequenos/PedidoRefatorado.java`
- `labs/m3/aula-090-de-main-gigante-para-metodos-pequenos/CadastroClienteRefatorado.java`
- `labs/m3/aula-090-de-main-gigante-para-metodos-pequenos/ProdutoRefatorado.java`
- `labs/m3/aula-090-de-main-gigante-para-metodos-pequenos/PagamentoRefatorado.java`
- `labs/m3/aula-090-de-main-gigante-para-metodos-pequenos/OrdemServicoRefatorada.java`
- `labs/m3/aula-090-de-main-gigante-para-metodos-pequenos/MensageriaRefatorada.java`
- `labs/m3/aula-090-de-main-gigante-para-metodos-pequenos/AuditoriaRefatorada.java`
- `labs/m3/aula-090-de-main-gigante-para-metodos-pequenos/README.md`
...
- deixar tudo no `main`;
- criar método com nome genérico;
- criar método que faz coisas demais;
- usar `void` para esconder cálculo;
- extrair método sem intenção;
- passar parâmetros demais sem perceber;
- refatorar tudo de uma vez;
- não rodar após refatorar;
- mudar comportamento sem perceber;
- achar que main pequeno sempre significa código bom.

### Próximo passo
Estudar assinatura de método profissional.
```
...
```text
1. O que é um main gigante?
2. Por que main gigante é problema?
3. O que é um método pequeno?
4. Método pequeno significa método artificial?
5. Quais sinais indicam que um trecho merece virar método?
6. Como um comentário pode denunciar método ausente?
7. O que significa responsabilidade de um método?
8. Para que servem parâmetros?
9. Quando um método deve retornar valor?
10. Quando faz sentido um método ser void?
11. Por que nomes genéricos são ruins?
12. Por que refatorar em passos pequenos?
13. Como validar que a refatoração preservou comportamento?
14. Por que muitos parâmetros podem ser sinal de problema futuro?
15. Qual diferença entre calcular e imprimir?
16. Por que cálculo deve normalmente retornar valor?
17. Como debug ajuda a entender métodos?
18. O que observar após extrair método?
19. Como isso prepara para orientação a objetos?
20. Qual foi a melhoria principal no PedidoRefatorado?
```
