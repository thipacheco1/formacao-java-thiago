...
Estamos no Módulo 1, estudando os fundamentos reais da linguagem Java.
...
Na aula anterior, aprendemos que variável é uma informação com nome e tipo.
...
Agora vamos aprofundar uma família específica de tipos:

```text
tipos inteiros.
```

Eles representam números sem casas decimais.
...
Em backend, números inteiros aparecem o tempo todo:
...
Saber escolher o tipo certo evita erro, confusão e bug silencioso.
...
## Diagnóstico de erro com inteiros

Quando der erro, siga o roteiro.

### 1. Qual tipo foi usado?
...
### 2. O valor cabe no tipo?

Compare com os limites.

### 3. O literal é grande demais para `int`?

Se for `long`, use `L`.

### 4. Há operação que pode estourar o limite?

Exemplo:

```java
int resultado = valorGrande + valorGrande;
```

### 5. O resultado deveria ser `long`?

Use `long` nos operandos também.

### 6. Tem decimal sendo colocado em inteiro?
...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println(maiorValor + 1);
    }
}
```

Compile, execute e observe o resultado.

### Teste 4 — Soma grande corrigida
...
### O que aprendi
Aprendi que Java possui tipos inteiros `byte`, `short`, `int` e `long`, cada um com limites próprios. Também aprendi que `int` é comum para quantidades e `long` é usado para valores maiores, como IDs e contadores massivos.

### O que pratiquei
Criei exemplos com tipos inteiros, consultei limites, observei overflow e apliquei os tipos em cenários de pedido, OS, auditoria, paginação, estoque e pagamento em centavos.
...
- IDs grandes
- valores em centavos
- escolha correta de tipo

### Arquivos criados
- `labs/m1/aula-025-tipos-inteiros/Main.java`
- `labs/m1/aula-025-tipos-inteiros/LimitesInteiros.java`
- `labs/m1/aula-025-tipos-inteiros/OverflowInteiro.java`
- `labs/m1/aula-025-tipos-inteiros/TentativasLogin.java`
- `labs/m1/aula-025-tipos-inteiros/Paginacao.java`
- `labs/m1/aula-025-tipos-inteiros/AuditoriaMassiva.java`
- `labs/m1/aula-025-tipos-inteiros/OrdemServicoInteiros.java`
- `labs/m1/aula-025-tipos-inteiros/ProdutoEstoque.java`
- `labs/m1/aula-025-tipos-inteiros/PagamentoCentavos.java`
...
- usar `byte` fora do limite;
- usar `short` fora do limite;
- esquecer `L` em literal `long`;
- usar `l` minúsculo;
- ignorar overflow;
- usar `int` para ID muito grande;
- usar vírgula em número;
- confundir inteiro com decimal;
- usar `byte` e `short` sem necessidade.

### Próximo passo
Estudar tipos decimais e primeiras limitações.
```
...
```text
1. Quais são os quatro tipos inteiros principais em Java?
2. Qual tipo inteiro é mais comum para quantidades simples?
3. Quando `long` costuma ser necessário?
4. Para que serve o sufixo `L`?
5. Por que é melhor usar `L` maiúsculo em vez de `l` minúsculo?
6. O que é overflow?
7. Overflow sempre gera erro?
8. Por que `byte` e `short` aparecem menos em código de negócio comum?
9. Por que um ID pode precisar de `long`?
10. Como representar R$ 99,90 em centavos usando inteiro?
```
