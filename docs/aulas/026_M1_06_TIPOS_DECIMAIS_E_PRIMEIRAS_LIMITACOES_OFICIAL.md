...
Estamos no Módulo 1, avançando nos fundamentos da linguagem Java.
...
Agora vamos estudar números com casas decimais.
...
Em Java, os dois tipos primitivos principais para valores decimais são:
...
Mas esta aula tem uma mensagem importante:

```text
número decimal em programação exige cuidado.
```
...
## Diagnóstico de erro com decimais

Quando der erro, siga o roteiro.

### 1. O número usa ponto ou vírgula?
...
Se não tiver, corrija.

### 4. O número está entre aspas?
...
### 5. O cálculo deveria ser decimal, mas saiu inteiro?

Verifique se a divisão foi feita entre inteiros.

### 6. Algum valor precisa ser convertido para `double` antes da divisão?
...
### 7. O resultado é `0.30000000000000004`?

Isso é limitação de ponto flutuante.

Não trate como bug de impressão apenas.

### 8. O domínio é dinheiro?

Considere centavos como inteiro ou, futuramente, `BigDecimal`.

### 9. A saída precisa de duas casas?

Use formatação, mas entenda que formatação não corrige cálculo.
...
### Teste 3 — Soma decimal inesperada
...
### Teste 4 — Valor monetário em centavos
...
| Ação | Atalho | Uso |
|---|---|---|
| Renomear variável | `Shift + F6` | Melhorar nomes |
...
```markdown
## Aula 026 — Tipos decimais e primeiras limitações

### O que aprendi
Aprendi que Java possui `float` e `double` para números decimais, que `double` é o padrão mais comum e que esses tipos possuem precisão aproximada.

### O que pratiquei
Criei exemplos com `double`, `float`, divisão inteira versus decimal, soma decimal inesperada, percentuais, médias, medidas e valores em centavos.
...
### Arquivos criados
- `labs/m1/aula-026-tipos-decimais/Main.java`
- `labs/m1/aula-026-tipos-decimais/FloatDouble.java`
- `labs/m1/aula-026-tipos-decimais/PrecisaoDecimal.java`
- `labs/m1/aula-026-tipos-decimais/DivisaoInteiraDecimal.java`
- `labs/m1/aula-026-tipos-decimais/MediaPedidos.java`
- `labs/m1/aula-026-tipos-decimais/DistanciaEntrega.java`
- `labs/m1/aula-026-tipos-decimais/MediaAvaliacao.java`
- `labs/m1/aula-026-tipos-decimais/PercentualConclusao.java`
- `labs/m1/aula-026-tipos-decimais/PedidoDecimal.java`
- `labs/m1/aula-026-tipos-decimais/PedidoCentavos.java`
- `labs/m1/aula-026-tipos-decimais/SlaAtendimento.java`
- `labs/m1/aula-026-tipos-decimais/PesoCubagem.java`
...
- esquecer `F` em literal `float`;
- colocar decimal em `int`;
- achar que `double` é exato;
- usar `double` para dinheiro real sem critério;
- fazer divisão inteira sem perceber;
- achar que `printf` corrige cálculo;
- ignorar domínio do número.

### Próximo passo
Estudar `boolean` e regras verdadeiras/falsas.
```
...
```text
1. Quais são os dois tipos decimais primitivos principais em Java?
2. Qual deles é o padrão mais comum para decimais?
3. Por que `float nota = 8.5;` pode dar erro?
4. Para que serve o sufixo `F`?
5. Por que Java usa ponto e não vírgula em literais decimais?
6. O que pode acontecer ao somar `0.1 + 0.2` com `double`?
7. Por que `double resultado = 10 / 4;` gera `2.0`?
8. Como fazer a divisão gerar `2.5`?
9. Por que dinheiro real exige cuidado com `double`?
10. Qual alternativa simples pode representar dinheiro em centavos?
```
