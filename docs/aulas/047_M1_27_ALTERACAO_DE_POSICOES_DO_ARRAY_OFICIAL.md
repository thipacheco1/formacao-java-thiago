...
Estamos no Módulo 1, continuando o bloco de arrays.
...
Na aula anterior, criamos arrays com tamanho definido pelo usuário:
...
Depois preenchemos as posições:
...
Agora vamos estudar o próximo passo:

```text
alterar um valor que já está dentro do array.
```
...
O array continua com o mesmo tamanho.

Mas o valor da posição 1 muda.
...
Essa aula é pequena no conceito, mas grande na importância.

Alterar posição de array é a base para atualização de dados em memória.
...
## Diagnóstico de alteração de array

Quando a alteração não funcionar, siga o roteiro.

### 1. Qual array está sendo alterado?

Verifique o nome da variável.

### 2. Qual índice está sendo usado?
...
### 4. A posição do usuário foi convertida para índice?

Se usuário usa 1, converta:

```java
indice = posicaoUsuario - 1;
```

### 5. Qual era o valor antigo?

Imprima antes.

### 6. Qual é o novo valor?

Imprima depois.

### 7. O novo valor precisa de validação?
...
### 8. Existe total ou média calculado antes?

Recalcule se necessário.

### 9. O loop está alterando uma ou várias posições?
...
Depois corrija validando o índice.

### Teste 2 — Posição do usuário usada como índice
...
Observe que alterou a segunda posição.
...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println(estoques[1]);
    }
}
```

Depois adicione validação para impedir estoque negativo.

### Teste 4 — Total desatualizado
...
### Teste 5 — Alterar todas as posições sem querer
...
Explique por que todos viraram 99.
...
| Ação | Atalho | Uso |
|---|---|---|
| Reformatar código | `Ctrl + Alt + L` | Organizar blocos de validação |
| Renomear variável | `Shift + F6` | Melhorar nomes como `indice`, `posicaoUsuario` |
| Terminal integrado | `Alt + F12` | Compilar e executar |
| Rodar programa | `Shift + F10` | Executar no IntelliJ |
| Debug | `Shift + F9` | Ver valor antes e depois |
...
### O que aprendi
Aprendi que arrays têm tamanho fixo, mas seus valores internos podem ser alterados por índice. Também aprendi a validar índice, guardar valor antigo, substituir valor, incrementar posição e tomar cuidado com valores padrão.

### O que pratiquei
Criei exemplos de alteração simples, alteração com for, valor antigo e novo, valores padrão, baixa de estoque, validação de índice, alteração via Scanner, ajuste de estoque, correção de pedido, atualização de OS, incremento de tentativa de mensageria, auditoria, ajuste de SLA, recalculo de total e alteração de várias posições.
...
- decremento de posição
- validação de índice
- `ArrayIndexOutOfBoundsException`
- recalcular total
- alteração segura
- valor derivado desatualizado

### Arquivos criados
- `labs/m1/aula-047-alteracao-posicoes-array/Main.java`
- `labs/m1/aula-047-alteracao-posicoes-array/AlteracaoComFor.java`
- `labs/m1/aula-047-alteracao-posicoes-array/ValorAntigoENovo.java`
- `labs/m1/aula-047-alteracao-posicoes-array/ValoresPadraoArray.java`
- `labs/m1/aula-047-alteracao-posicoes-array/AlterandoValorPadrao.java`
- `labs/m1/aula-047-alteracao-posicoes-array/BaixaEstoqueArray.java`
- `labs/m1/aula-047-alteracao-posicoes-array/BaixaEstoqueSegura.java`
- `labs/m1/aula-047-alteracao-posicoes-array/ValidacaoIndice.java`
- `labs/m1/aula-047-alteracao-posicoes-array/AlterarPosicaoConsole.java`
- `labs/m1/aula-047-alteracao-posicoes-array/AlterarPosicaoComHistorico.java`
- `labs/m1/aula-047-alteracao-posicoes-array/AjusteEstoqueProduto.java`
- `labs/m1/aula-047-alteracao-posicoes-array/CorrigirValorPedido.java`
- `labs/m1/aula-047-alteracao-posicoes-array/AtualizarAtividadesOs.java`
- `labs/m1/aula-047-alteracao-posicoes-array/IncrementarTentativaMensagem.java`
- `labs/m1/aula-047-alteracao-posicoes-array/RegistrarEventoAuditoriaArray.java`
- `labs/m1/aula-047-alteracao-posicoes-array/AjustarSlaHoras.java`
- `labs/m1/aula-047-alteracao-posicoes-array/RecalcularTotalAposAlteracao.java`
- `labs/m1/aula-047-alteracao-posicoes-array/AjustarTotalComValorAntigo.java`
- `labs/m1/aula-047-alteracao-posicoes-array/AumentarTodosValores.java`
- `labs/m1/aula-047-alteracao-posicoes-array/ZerarValoresNegativos.java`
- `labs/m1/aula-047-alteracao-posicoes-array/AplicarLimiteMaximo.java`
- `labs/m1/aula-047-alteracao-posicoes-array/SubstituirZeros.java`
- `labs/m1/aula-047-alteracao-posicoes-array/ErroIndiceInvalido.java`
- `labs/m1/aula-047-alteracao-posicoes-array/ErroPosicaoUsuarioComoIndice.java`
- `labs/m1/aula-047-alteracao-posicoes-array/ErroTotalDesatualizado.java`
...
- achar que alterar valor muda o tamanho do array;
- usar posição do usuário direto como índice;
- não validar índice;
- não validar novo valor;
- perder valor antigo necessário;
- não recalcular valores derivados depois da alteração;
- confundir valor padrão com dado preenchido;
- usar `<= array.length`;
- alterar todas as posições quando queria alterar uma;
- alterar uma posição e não conferir o resultado.

### Próximo passo
Estudar busca em array.
```
...
```text
1. Alterar uma posição do array muda o tamanho dele?
2. O que acontece com o valor antigo quando fazemos `array[indice] = novoValor`?
3. Quando vale a pena guardar o valor antigo?
4. O que são default values em arrays numéricos?
5. Qual é o intervalo válido de índices de um array?
6. Como validar se um índice é válido?
7. Qual a diferença entre posição do usuário e índice técnico?
8. Por que valores derivados, como total e média, podem ficar desatualizados após alteração?
9. Quando uma alteração incremental é melhor que uma substituição direta?
10. Por que zero pode ser um valor padrão e também um valor legítimo do domínio?
```
