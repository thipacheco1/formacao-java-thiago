...
Estamos no Módulo 1, consolidando a base da linguagem Java.
...
Na aula anterior, vimos a diferença entre:
...
Agora vamos aprofundar `String`.

Não vamos estudar todos os detalhes internos de `String` ainda.

Mas vamos aprender os métodos básicos que já aparecem em validações, regras simples e leitura de dados:
...
Esses métodos são pequenos, mas muito importantes.

Eles aparecem o tempo todo em código backend.
...
## Diagnóstico de erro com String

Quando der problema, siga o roteiro.

### 1. A variável é `String`?
...
### 3. A comparação usa `equals`?

Evite `==`.

### 4. A comparação deve ignorar maiúsculas/minúsculas?

Use `equalsIgnoreCase()` se a regra permitir.

### 5. Campo com espaços deve ser inválido?

Use `isBlank()`.

### 6. Chamou `trim()` e guardou o retorno?

Se não guardou, o texto original permanece.

### 7. `contains()` falhou por caixa?

Padronize com `toLowerCase()` ou `toUpperCase()` antes.

### 8. A concatenação virou resultado estranho?

Verifique ordem e parênteses.

### 9. A String pode ser `null`?

Não chame métodos sem tratar.

### 10. O erro aponta linha?

Leia a mensagem do compilador ou da exceção.
...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println(status.equals("ABERTA"));
    }
}
```
...
Não se preocupe ainda com `new String`.

O objetivo é ver que `==` não é comparação de conteúdo.

### Teste 2 — `trim()` sem guardar
...
### Teste 3 — `isEmpty()` versus `isBlank()`
...
| Ação | Atalho | Uso |
|---|---|---|
| Renomear variável | `Shift + F6` | Melhorar nomes de texto |
...
```markdown
## Aula 029 — String básica

### O que aprendi
Aprendi métodos básicos de `String`, incluindo concatenação, `length`, `isEmpty`, `isBlank`, `equals`, `equalsIgnoreCase`, `contains` e `trim`.

### O que pratiquei
Criei validações simples de texto aplicadas a cliente, pedido, OS, produto, mensagem e auditoria.
...
### Arquivos criados
- `labs/m1/aula-029-string-basica/Main.java`
- `labs/m1/aula-029-string-basica/ComparacaoString.java`
- `labs/m1/aula-029-string-basica/ValidacaoTexto.java`
- `labs/m1/aula-029-string-basica/ClienteStringBasica.java`
- `labs/m1/aula-029-string-basica/PedidoStringBasica.java`
- `labs/m1/aula-029-string-basica/OrdemServicoStringBasica.java`
- `labs/m1/aula-029-string-basica/ProdutoStringBasica.java`
- `labs/m1/aula-029-string-basica/MensagemStringBasica.java`
- `labs/m1/aula-029-string-basica/AuditoriaStringBasica.java`
...
- comparar String com `==`;
- esquecer que `equals` diferencia maiúsculas/minúsculas;
- usar `isEmpty()` quando a regra exige `isBlank()`;
- chamar `trim()` sem guardar retorno;
- achar que `contains()` ignora caixa;
- validar e-mail apenas com `contains("@")` como se fosse validação completa;
- concatenar sem espaço;
- soma virar concatenação;
- chamar método em String `null`;
- encadear métodos demais e perder leitura.

### Próximo passo
Estudar entrada de dados com Scanner.
```
...
```text
1. Para que serve `length()`?
2. Qual a diferença entre `isEmpty()` e `isBlank()`?
3. Para que serve `trim()`?
4. Por que `trim()` precisa ter o retorno guardado?
5. Por que não devemos comparar String com `==` como regra geral?
6. Para que serve `equals()`?
7. Para que serve `equalsIgnoreCase()`?
8. Para que serve `contains()`?
9. `contains()` diferencia maiúsculas/minúsculas?
10. Por que `"Resultado: " + a + b` pode gerar resultado inesperado?
```
