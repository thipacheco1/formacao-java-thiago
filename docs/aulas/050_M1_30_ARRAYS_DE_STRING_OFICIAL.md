...
Estamos no Módulo 1, saindo dos arrays apenas numéricos e entrando em arrays de texto.
...
Até agora, trabalhamos bastante com números:
...
Agora vamos trabalhar com texto:

```java
String[] nomes = {"Ana", "Bruno", "Carla"};
```

Arrays de `String` são usados para guardar listas de textos.
...
Essa aula é importante porque texto tem regras próprias.
...
Com `String`, a comparação correta normalmente é:

```java
equals()
```

E a validação textual usa métodos como:
...
## Diagnóstico de arrays de String

Quando o array de `String` não funcionar, siga o roteiro.

### 1. O array foi preenchido?
...
as posições começam `null`.

### 2. O índice está correto?
...
### 3. Você está comparando String com equals?

Use:

```java
"VALOR".equals(variavel)
```

quando possível.

### 4. O texto pode estar com espaços?
...
### 5. A comparação precisa ignorar caixa?
...
### 6. O texto pode estar vazio ou em branco?
...
### 8. Você misturou nextInt e nextLine?

Limpe a quebra pendente.

### 9. A mensagem de erro explica a regra?

Exemplo:

```text
Status inválido. Use PENDENTE, APROVADO, RECUSADO ou CANCELADO.
```
...
### Teste 5 — nextInt antes de nextLine
...
| Ação | Atalho | Uso |
|---|---|---|
| Reformatar código | `Ctrl + Alt + L` | Organizar loops e ifs |
| Renomear variável | `Shift + F6` | Melhorar nomes como `statusNormalizado` |
| Terminal integrado | `Alt + F12` | Compilar e executar |
| Rodar programa | `Shift + F10` | Executar no IntelliJ |
| Debug | `Shift + F9` | Ver texto bruto e normalizado |
...
### O que aprendi
Aprendi a criar, preencher, percorrer, validar, buscar e alterar arrays de String, usando `equals`, `isEmpty`, `isBlank`, `trim` e normalização textual.

### O que pratiquei
Criei listas de nomes, produtos, clientes, status, filas, usuários de auditoria e tipos de mensagem. Também validei textos vazios, status inválidos, normalizei entradas, contei status, busquei nomes e alterei posições textuais.
...
- `length`
- `null`
- valor padrão de array de String
- `isEmpty`
- `isBlank`
- `equals`
- `equalsIgnoreCase`
...
- comparação segura
- busca textual
- alteração de texto no array

### Arquivos criados
- `labs/m1/aula-050-arrays-string/Main.java`
- `labs/m1/aula-050-arrays-string/PercorrendoNomes.java`
- `labs/m1/aula-050-arrays-string/CriandoArrayString.java`
- `labs/m1/aula-050-arrays-string/ArrayStringComNull.java`
- `labs/m1/aula-050-arrays-string/IsEmptyVsIsBlank.java`
- `labs/m1/aula-050-arrays-string/BuscaNomeEquals.java`
- `labs/m1/aula-050-arrays-string/PreencherNomesConsole.java`
- `labs/m1/aula-050-arrays-string/ValidarNomesArray.java`
- `labs/m1/aula-050-arrays-string/NomesComTamanhoUsuario.java`
- `labs/m1/aula-050-arrays-string/ClientesArray.java`
- `labs/m1/aula-050-arrays-string/ProdutosArray.java`
- `labs/m1/aula-050-arrays-string/StatusPedidosArray.java`
- `labs/m1/aula-050-arrays-string/ContarStatusAprovado.java`
- `labs/m1/aula-050-arrays-string/RelatorioStatusPedidos.java`
- `labs/m1/aula-050-arrays-string/ValidarStatusPedidos.java`
- `labs/m1/aula-050-arrays-string/NormalizarStatusArray.java`
- `labs/m1/aula-050-arrays-string/BuscarStatusArray.java`
- `labs/m1/aula-050-arrays-string/BuscarNomeNormalizado.java`
- `labs/m1/aula-050-arrays-string/BuscarNomeEqualsIgnoreCase.java`
- `labs/m1/aula-050-arrays-string/PrimeiroTextoEmBranco.java`
- `labs/m1/aula-050-arrays-string/ValidarTodosOsNomes.java`
- `labs/m1/aula-050-arrays-string/AuditoriaUsuariosArray.java`
- `labs/m1/aula-050-arrays-string/MensageriaTiposArray.java`
- `labs/m1/aula-050-arrays-string/FilasAtendimentoArray.java`
- `labs/m1/aula-050-arrays-string/ValidarFilaAtendimento.java`
- `labs/m1/aula-050-arrays-string/RelatorioStatusInvalidos.java`
- `labs/m1/aula-050-arrays-string/RelatorioStatusInvalidosMelhorado.java`
- `labs/m1/aula-050-arrays-string/AlterarNomeArray.java`
- `labs/m1/aula-050-arrays-string/BuscarEAlterarStatus.java`
- `labs/m1/aula-050-arrays-string/ErroStringComIgualIgual.java`
- `labs/m1/aula-050-arrays-string/ErroIsBlankEmNull.java`
- `labs/m1/aula-050-arrays-string/ErroIsEmptyComEspacos.java`
- `labs/m1/aula-050-arrays-string/ErroSemTrim.java`
- `labs/m1/aula-050-arrays-string/ErroNextIntNextLine.java`
...
- comparar String com `==`;
- chamar `isBlank` em valor `null`;
- confundir `isEmpty` com `isBlank`;
- não usar `trim` em entrada do usuário;
- não normalizar antes de comparar status;
- esquecer `scanner.nextLine()` depois de `nextInt`;
- achar que `new String[n]` cria Strings vazias;
- usar `equals` do lado que pode ser `null`;
- não validar textos antes de salvar no array;
- misturar índice técnico e posição do usuário.

### Próximo passo
Estudar arrays paralelos.
```
...
```text
1. O que é um array de String?
2. Qual é o valor padrão de uma posição em `new String[n]`?
3. Qual a diferença entre `null`, `""` e `"   "`?
4. Qual a diferença entre `isEmpty()` e `isBlank()`?
5. Por que String deve ser comparada com `equals()`?
6. Por que `"APROVADO".equals(status)` pode ser mais seguro?
7. Para que serve `trim()`?
8. Para que serve normalizar texto com `toUpperCase()` ou `toLowerCase()`?
9. Qual cuidado existe ao misturar `nextInt()` e `nextLine()`?
10. Como buscar um texto dentro de um array de String?
```
