...
Na aula anterior, estudamos text blocks.

Agora vamos estudar um dos assuntos mais importantes para backend profissional:

```text
exceptions.
```

Não vamos apenas decorar `try/catch`.
...
como descobrir a causa raiz;
qual diferença entre checked e unchecked;
como escrever mensagens melhores;
quando capturar;
quando propagar;
quando relançar;
quando não engolir erro;
como diagnosticar erros em backend.
```
...
## Diagnóstico de exception

Quando aparecer uma exception, pergunte:
...
### 3. Qual é o primeiro `at` do seu código?

Comece por ali.

### 4. Existe `Caused by`?

A causa raiz pode estar lá.

### 5. A exceção foi relançada?

Verifique se preservou causa.

### 6. O erro é de argumento ou estado?
...
### 7. O erro é técnico ou de negócio?

Técnico:

```text
arquivo, banco, rede, parse.
```

Negócio:

```text
status inválido, regra violada, campo obrigatório.
```

### 8. Onde deve ser tratado?

Na camada atual ou superior?

### 9. A mensagem é boa?

Se não, melhore.

### 10. O teste cobre o erro?

Erro importante deve ter teste no futuro.
...
Capture e ignore uma exception.

Explique por que isso é perigoso.

---

### Teste 5 — relançar sem causa
...
```java
throw new RuntimeException("Erro.", erroOriginal);
```

Explique a diferença no stack trace.
...
Entender exceções em Java por baixo, incluindo stack trace, checked exceptions, unchecked exceptions, causa raiz, mensagens de erro, propagação, captura, relançamento e diagnóstico.

## Conceitos

- Exception representa uma falha ou condição excepcional.
- Stack trace mostra o caminho de chamadas até o erro.
- O primeiro `at` do seu código geralmente aponta o início da investigação.
- Checked exception exige tratamento ou declaração.
- Unchecked exception não exige tratamento pelo compilador.
- `throw` lança exceção.
- `throws` declara exceção.
- `try/catch` captura exceção.
- `finally` executa limpeza.
- `getMessage` retorna a mensagem.
- `printStackTrace` mostra a pilha, mas não é solução final de logging.
- Causa raiz pode aparecer como `Caused by`.
- Ao relançar, preservar causa é essencial.
- `IllegalArgumentException` indica argumento inválido.
- `IllegalStateException` indica estado inválido.
- Catch vazio é perigoso.
- Mensagens devem ajudar o diagnóstico.
...
| Debug | `Shift + F9` | Parar no throw |
| Run | `Shift + F10` | Executar normal |
| Step Over | `F8` em muitos keymaps | Avançar validações |
| Step Into | `F7` em muitos keymaps | Entrar no método que falha |
| Variables | janela Debug | Ver valor inválido |
| Evaluate Expression | `Alt + F8` em muitos keymaps | Testar validações |
| Reformatar código | `Ctrl + Alt + L` | Organizar |
| Renomear | `Shift + F6` | Melhorar nomes |
| Extrair método | `Ctrl + Alt + M` em muitos keymaps | Separar validação |
...
### O que aprendi
Aprendi a ler stack trace, identificar tipo da exceção, mensagem, linha de origem, cadeia de chamadas e causa raiz. Também aprendi a diferença entre checked e unchecked exceptions, quando lançar, capturar, propagar e relançar preservando a causa.

### O que pratiquei
Criei exemplos com divisão por zero, NullPointerException, IllegalArgumentException, IllegalStateException, NumberFormatException, try/catch, finally, throws, propagação, causa raiz, exception própria, ordem de catch, multi-catch e aplicações em cliente, produto, pedido, pagamento, OS, mensageria e auditoria.
...
### Arquivos criados
- `labs/m2/aula-085-exceptions-por-baixo/Main.java`
- `labs/m2/aula-085-exceptions-por-baixo/StackTraceCamadas.java`
- `labs/m2/aula-085-exceptions-por-baixo/NullPointerBasico.java`
- `labs/m2/aula-085-exceptions-por-baixo/IllegalArgumentBasico.java`
- `labs/m2/aula-085-exceptions-por-baixo/IllegalStateBasico.java`
- `labs/m2/aula-085-exceptions-por-baixo/NumberFormatBasico.java`
- `labs/m2/aula-085-exceptions-por-baixo/TryCatchBasico.java`
- `labs/m2/aula-085-exceptions-por-baixo/GetMessageExemplo.java`
- `labs/m2/aula-085-exceptions-por-baixo/PrintStackTraceExemplo.java`
- `labs/m2/aula-085-exceptions-por-baixo/FinallyBasico.java`
- `labs/m2/aula-085-exceptions-por-baixo/ThrowsBasico.java`
- `labs/m2/aula-085-exceptions-por-baixo/PropagacaoException.java`
- `labs/m2/aula-085-exceptions-por-baixo/CausaException.java`
- `labs/m2/aula-085-exceptions-por-baixo/ExceptionPropria.java`
- `labs/m2/aula-085-exceptions-por-baixo/OrdemCatch.java`
- `labs/m2/aula-085-exceptions-por-baixo/MultiCatchExemplo.java`
- `labs/m2/aula-085-exceptions-por-baixo/ClienteException.java`
- `labs/m2/aula-085-exceptions-por-baixo/ProdutoException.java`
- `labs/m2/aula-085-exceptions-por-baixo/PedidoException.java`
- `labs/m2/aula-085-exceptions-por-baixo/PagamentoException.java`
- `labs/m2/aula-085-exceptions-por-baixo/OrdemServicoException.java`
- `labs/m2/aula-085-exceptions-por-baixo/MensageriaException.java`
- `labs/m2/aula-085-exceptions-por-baixo/AuditoriaException.java`
- `labs/m2/aula-085-exceptions-por-baixo/DebugException.java`
- `labs/m2/aula-085-exceptions-por-baixo/ErroCatchVazio.java`
- `labs/m2/aula-085-exceptions-por-baixo/ErroMensagemGenerica.java`
- `labs/m2/aula-085-exceptions-por-baixo/ErroPerderCausaRaiz.java`
- `labs/m2/aula-085-exceptions-por-baixo/ErroCatchGenerico.java`
- `labs/m2/aula-085-exceptions-por-baixo/README.md`
...
- perder causa raiz;
- capturar `Exception` genérico sem necessidade;
- usar exception para fluxo normal simples;
- mostrar stack trace cru para usuário final;
- não validar entrada cedo;
- criar exception própria para tudo;
- misturar regra de negócio com erro técnico sem contexto;
- ignorar stack trace.

### Próximo passo
Estudar entrada/saída básica com console robusto.
```
...
3. Como identificar o tipo da exceção?
4. Como identificar a mensagem da exceção?
5. Onde começar a ler o stack trace?
6. O que significa Caused by?
7. O que é causa raiz?
8. Qual a diferença entre checked e unchecked?
9. O que significa throw?
10. O que significa throws?
11. Para que serve try/catch?
12. Para que serve finally?
13. Quando usar IllegalArgumentException?
14. Quando usar IllegalStateException?
15. Por que catch vazio é perigoso?
16. Por que preservar causa ao relançar?
17. Quando capturar exceção?
18. Quando propagar exceção?
19. O que faz uma mensagem de erro ser boa?
20. Por que stack trace é mapa de investigação?
```
