...
O conteúdo original já explica o papel do IntelliJ, SDK, projeto, source root, run configuration, terminal integrado e uso profissional da IDE. Este complemento apenas deixa explícito o fluxo operacional de download e instalação.

### Fonte de download

Baixe pelo site oficial da JetBrains.

A edição da formação é:

```text
IntelliJ IDEA Community Edition
```

A edição Community é suficiente para esta fase porque permite:
...
```text
1. Baixar IntelliJ IDEA Community Edition.
2. Executar o instalador.
3. Manter opções padrão quando não houver necessidade específica.
4. Criar atalho, se desejar.
5. Finalizar a instalação.
6. Abrir o IntelliJ.
7. Não instalar plugins aleatórios no primeiro acesso.
8. Configurar ou confirmar o JDK do projeto.
```

### Configurar o JDK no IntelliJ

Se a IDE não detectar o JDK automaticamente:
...
Selecione a raiz do JDK validada na aula 005.
...
Project
SDK: JDK 21
Language level: compatível com o JDK usado
Apply
OK
```
...
A IDE deve abrir a raiz do projeto.
...
public class Main {
    public static void main(String[] args) {
        System.out.println("IntelliJ IDEA Community configurado.");
    }
}
```
...
Se a IDE acusar erro:

```text
verifique Project SDK;
verifique se o arquivo está em src;
verifique se src é source root;
verifique se há método main correto;
verifique se a run configuration aponta para a classe certa.
```

Se funciona no terminal e não na IDE:

```text
problema provável: SDK/configuração da IDE.
```

Se funciona na IDE e não no terminal:

```text
problema provável: PATH/JAVA_HOME no Windows.
```
