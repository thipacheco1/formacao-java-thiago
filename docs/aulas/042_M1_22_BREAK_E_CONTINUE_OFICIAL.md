...
Estamos no Módulo 1, ainda dentro do bloco de repetição.
...
Nas últimas aulas, aprendemos a repetir:
...
Agora vamos estudar duas palavras que alteram o fluxo dentro de uma repetição:
...
A diferença entre elas é simples, mas muito importante:
...
## Diagnóstico de erro com break e continue

Quando o fluxo não bater, siga este roteiro.

### 1. A intenção era parar tudo ou pular item?
...
### 2. O comando está dentro de qual estrutura?
...
### 3. O break está saindo da estrutura que você imaginava?

Se estiver dentro de `switch`, talvez não saia do loop externo.

### 4. O continue está pulando alguma atualização importante?

Em `while`, confira o contador.

### 5. O item ignorado está sendo contado?

Se precisa relatório, incremente contador de ignorados.

### 6. O motivo da parada está registrado?

Se é erro crítico, registre mensagem.

### 7. A condição do loop poderia resolver sem break?

Se sim, simplifique.

### 8. Há muitos continues?

Talvez precise reorganizar a regra.

### 9. A ordem entre break e continue está correta?

Condições críticas primeiro.

### 10. Testou todos os cenários?

Teste item válido, item inválido e parada crítica.
...
### Teste 1 — Break no lugar de continue
...
Depois troque `break` por `continue`.

Compare.

### Teste 2 — Continue perigoso em while
...
### Teste 3 — Break dentro de switch dentro de for
...
Observe que o `for` continua.

### Teste 4 — Continue como filtro
...
Explique por que só imprime pares.

### Teste 5 — Break e continue juntos
...
| Ação | Atalho | Uso |
|---|---|---|
| Reformatar código | `Ctrl + Alt + L` | Organizar blocos e condições |
| Renomear variável | `Shift + F6` | Nomear motivos de parada/ignorado |
...
| Debug | `Shift + F9` | Ver break e continue em ação |
...
### O que aprendi
Aprendi que `break` interrompe a repetição inteira, enquanto `continue` pula apenas a iteração atual e segue para a próxima.

### O que pratiquei
Criei exemplos mínimos e aplicados em pedido, lote, mensageria, auditoria, produto, pagamento, menu, soma de valores válidos e filtro de pares.
...
- contadores de ignorados
- motivo de parada

### Arquivos criados
- `labs/m1/aula-042-break-continue/Main.java`
- `labs/m1/aula-042-break-continue/ContinueMinimo.java`
- `labs/m1/aula-042-break-continue/BreakWhile.java`
- `labs/m1/aula-042-break-continue/ContinueWhile.java`
- `labs/m1/aula-042-break-continue/BreakDoWhile.java`
- `labs/m1/aula-042-break-continue/ContinueDoWhile.java`
- `labs/m1/aula-042-break-continue/BuscaPedidoBreak.java`
- `labs/m1/aula-042-break-continue/PedidosCanceladosContinue.java`
- `labs/m1/aula-042-break-continue/LoteErroCriticoBreak.java`
- `labs/m1/aula-042-break-continue/LoteItemInvalidoContinue.java`
- `labs/m1/aula-042-break-continue/MensageriaContinue.java`
- `labs/m1/aula-042-break-continue/MensageriaBreak.java`
- `labs/m1/aula-042-break-continue/AuditoriaContinue.java`
- `labs/m1/aula-042-break-continue/ProdutosInativosContinue.java`
- `labs/m1/aula-042-break-continue/PagamentosContinue.java`
- `labs/m1/aula-042-break-continue/PagamentosBreak.java`
- `labs/m1/aula-042-break-continue/MenuBreak.java`
- `labs/m1/aula-042-break-continue/SomaValoresValidosContinue.java`
- `labs/m1/aula-042-break-continue/BreakEContinueMesmoLoop.java`
- `labs/m1/aula-042-break-continue/BreakSwitchDentroFor.java`
- `labs/m1/aula-042-break-continue/ContinueFiltroPares.java`
- `labs/m1/aula-042-break-continue/ContinuePerigosoWhile.java`
...
- usar `break` quando queria `continue`;
- usar `continue` quando deveria parar tudo;
- usar `continue` antes de atualizar contador em `while`;
- usar `break` para esconder condição ruim;
- usar `continue` demais e deixar fluxo confuso;
- achar que `break` dentro de `switch` sai do loop externo;
- ignorar item sem contar como ignorado;
- parar processamento sem registrar motivo;
- usar `break` e `continue` para todo controle;
- não testar cenário pulado e cenário parado.

### Próximo passo
Estudar laços aninhados.
```
...
```text
1. Para que serve o `break`?
2. Para que serve o `continue`?
3. Qual a diferença prática entre `break` e `continue`?
4. Quando faz sentido usar `break` em um processamento?
5. Quando faz sentido usar `continue` em um processamento?
6. Por que `continue` em `while` pode causar loop infinito?
7. O que acontece com `break` dentro de um `switch` que está dentro de um `for`?
8. Por que devemos registrar o motivo de uma parada com `break`?
9. Por que devemos contar itens ignorados com `continue` em muitos cenários?
10. Quando a condição do próprio loop é melhor do que usar `break`?
```
