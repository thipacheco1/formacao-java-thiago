...
Estamos no Módulo 1, avançando no bloco de arrays.
...
Na aula anterior, criamos arrays com tamanho fixo diretamente no código:
...
O problema é que, em sistema real, nem sempre sabemos antes quantos valores serão informados.

Às vezes o usuário decide.

Exemplo:

```text
quantos pedidos deseja cadastrar?
quantos pagamentos deseja informar?
quantas notas deseja digitar?
quantas atividades existem na OS?
quantas mensagens serão processadas?
quantos eventos de auditoria serão avaliados?
```

Nesses casos, primeiro lemos a quantidade.

Depois criamos o array com esse tamanho.
...
Essa é a ideia central desta aula.
...
Quando o programa com array de tamanho informado der errado, siga o roteiro.

### 1. A quantidade foi lida corretamente?
...
### 2. A quantidade foi validada antes de criar o array?

Verifique a ordem.

### 3. A quantidade pode ser zero?

Se sim, o restante do código trata array vazio?

### 4. Existe limite máximo?

Se não, deveria existir?

### 5. O array foi criado com o tamanho validado?
...
### 6. O loop usa `< array.length`?

Esse é o padrão.

### 7. Cada posição foi preenchida?

Verifique o loop de preenchimento.

### 8. Cada valor precisa de validação própria?

Quantidade válida não garante valor válido.

### 9. O código acessa `[0]`?

Garanta que o array não esteja vazio.
...
Depois corrija validando a quantidade antes.

### Teste 2 — Criar antes de validar
...
Explique por que a validação veio tarde demais.
...
### Teste 4 — Boolean não atualizado
...
Digite valor inválido e veja o problema.

Depois atualize o boolean dentro do loop.

### Teste 5 — Índice do usuário versus índice técnico
...
| Ação | Atalho | Uso |
|---|---|---|
| Reformatar código | `Ctrl + Alt + L` | Organizar validação e loops |
| Renomear variável | `Shift + F6` | Melhorar nomes como `quantidade`, `indice` |
| Terminal integrado | `Alt + F12` | Compilar e executar |
| Rodar programa | `Shift + F10` | Executar no IntelliJ |
| Parar execução | botão Stop ou `Ctrl + F2` em muitos keymaps | Interromper loop incorreto |
| Debug | `Shift + F9` | Ver quantidade, length e índice |
...
```markdown
## Aula 046 — Arrays com tamanho definido pelo usuário

### O que aprendi
Aprendi a criar arrays com tamanho informado em tempo de execução, validando a quantidade antes de usar `new double[n]`, `new int[n]` ou `new long[n]`.

### O que pratiquei
Criei arrays com tamanho informado pelo usuário, preenchi com Scanner, validei quantidade, validei cada elemento, calculei soma, média, maior, menor e apliquei em notas, pedidos, produtos, OS, mensageria, auditoria, pagamentos e SLA.
...
- boolean de validação

### Arquivos criados
- `labs/m1/aula-046-arrays-tamanho-usuario/Main.java`
- `labs/m1/aula-046-arrays-tamanho-usuario/NotasComMedia.java`
- `labs/m1/aula-046-arrays-tamanho-usuario/MaiorMenorNota.java`
- `labs/m1/aula-046-arrays-tamanho-usuario/PedidosComTamanhoUsuario.java`
- `labs/m1/aula-046-arrays-tamanho-usuario/ProdutosComTamanhoUsuario.java`
- `labs/m1/aula-046-arrays-tamanho-usuario/AtividadesPorOsComTamanhoUsuario.java`
- `labs/m1/aula-046-arrays-tamanho-usuario/MensageriaComTamanhoUsuario.java`
- `labs/m1/aula-046-arrays-tamanho-usuario/AuditoriaComTamanhoUsuario.java`
- `labs/m1/aula-046-arrays-tamanho-usuario/PagamentosComTamanhoUsuario.java`
- `labs/m1/aula-046-arrays-tamanho-usuario/SlaComTamanhoUsuario.java`
- `labs/m1/aula-046-arrays-tamanho-usuario/TamanhoComLimiteMaximo.java`
- `labs/m1/aula-046-arrays-tamanho-usuario/NotasComDoWhile.java`
- `labs/m1/aula-046-arrays-tamanho-usuario/ErroTamanhoNegativo.java`
- `labs/m1/aula-046-arrays-tamanho-usuario/ErroCriarAntesDeValidar.java`
- `labs/m1/aula-046-arrays-tamanho-usuario/ErroLoopComMenorIgual.java`
- `labs/m1/aula-046-arrays-tamanho-usuario/ErroBooleanNaoAtualizado.java`
...
- criar array antes de validar quantidade;
- permitir quantidade negativa;
- permitir quantidade zero e acessar índice 0;
- usar `<= array.length`;
- usar `quantidade` no loop quando `array.length` comunica melhor;
- não validar cada valor preenchido;
- não limitar tamanho máximo quando necessário;
- confundir índice técnico com número do usuário;
- esquecer de atualizar boolean de validação;
- tentar adicionar além do tamanho fixo.

### Próximo passo
Estudar alteração de posições do array.
```
...
```text
1. O que significa criar array com tamanho definido pelo usuário?
2. Por que precisamos validar a quantidade antes de usar `new double[n]`?
3. O que pode acontecer se a quantidade for negativa?
4. Por que quantidade zero pode ser perigosa quando usamos `array[0]`?
5. Por que é melhor percorrer com `array.length`?
6. Qual a diferença entre índice técnico e número exibido para o usuário?
7. Por que pode ser importante definir um limite máximo?
8. O que acontece se uma variável booleana de validação não for atualizada dentro do loop?
9. Por que validar o tamanho não substitui validar cada valor do array?
10. Por que array com tamanho definido pelo usuário ainda continua tendo tamanho fixo depois de criado?
```
