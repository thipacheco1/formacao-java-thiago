...
Na aula anterior, vimos o caminho:

```text
.java -> javac -> .class -> JVM -> execução
```

Agora vamos olhar para dentro da execução.

Quando um programa roda na JVM, dados precisam ficar em algum lugar.
...
```text
onde fica a variável quantidade?
onde fica o array?
onde fica a referência para o array?
o que acontece quando passo um array para um método?
por que um int alterado dentro do método não muda fora?
por que um array alterado dentro do método muda fora?
o que é null?
por que NullPointerException acontece?
quando uma variável local deixa de existir?
quando um objeto pode ser descartado?
```

Essas perguntas são respondidas com três conceitos:
...
Essa aula é essencial para entender Java de verdade.
...
## Diagnóstico de stack, heap e referências

Quando algo parecer estranho, siga o roteiro.

### 1. A variável é primitiva ou referência?
...
### 2. Onde a variável foi declarada?
...
### 3. Ela está na stack ou aponta para heap?

Variável local está no frame.

Se for referência, aponta para objeto no heap.

### 4. O método alterou o conteúdo do objeto?
...
### 5. O método apenas reatribuiu o parâmetro?
...
Isso não muda a variável do chamador.

### 6. Existe null?

Antes de usar, valide.

### 7. Existem duas referências para o mesmo objeto?

Se sim, alteração por uma aparece pela outra.

### 8. O objeto foi criado e perdido?

Se ninguém guarda referência, ele fica inacessível.

### 9. O nome do método comunica alteração?

Se não, renomeie.

### 10. Use debug

Observe referências, campos e valores antes/depois da chamada.
...
### Teste 1 — Copiar referência achando que clonou
...
### Teste 2 — Reatribuir parâmetro esperando trocar fora
...
public class Main {
    ...
    public static void trocar(Cliente cliente) {
        cliente = new Cliente();
        cliente.nome = "Bruno";
    }
}
...
Depois corrija com validação de null.

---

### Teste 4 — String sem retorno
...
Depois corrija com retorno.

---

### Teste 5 — Objeto criado e perdido
...
Explique por que não é possível acessar esse cliente no `main`.
...
```markdown
# Aula 063 — Stack, heap e referências

## Objetivo

Entender como variáveis locais, parâmetros, primitivos, referências, arrays, objetos e `null` se comportam na memória durante a execução de programas Java.

## Conceitos

- Stack guarda frames de métodos.
- Variáveis locais e parâmetros vivem no frame do método.
- Heap guarda objetos e arrays.
- Variáveis de referência apontam para objetos no heap.
- Primitivos guardam valores.
- Array é objeto.
- String é objeto imutável.
- Reatribuir parâmetro não altera variável do chamador.
- Alterar conteúdo de objeto compartilhado aparece para todos que apontam para ele.
- `null` significa ausência de referência.
- Chamar método em `null` causa `NullPointerException`.
- Objeto sem referência alcançável fica elegível para coleta de lixo.
...
| Debug | `Shift + F9` | Ver referências e campos |
...
| Variables | janela do Debug | Ver variáveis e campos |
...
### O que aprendi
Aprendi que a stack guarda frames de métodos, variáveis locais e parâmetros, enquanto o heap guarda objetos e arrays. Também aprendi que variáveis de referência apontam para objetos no heap e que `null` representa ausência de referência.

### O que pratiquei
Criei exemplos com primitivos, arrays, String, StringBuilder e classes simples como Cliente, Pedido, Produto, Pagamento, OrdemServico, Mensagem e RegistroAuditoria. Também provoquei NullPointerException, reatribuição de referência, mutação de objeto e cópia de referência.
...
### Arquivos criados
- `labs/m2/aula-063-stack-heap-referencias/Main.java`
- `labs/m2/aula-063-stack-heap-referencias/StackPrimitivo.java`
- `labs/m2/aula-063-stack-heap-referencias/HeapArray.java`
- `labs/m2/aula-063-stack-heap-referencias/ReatribuicaoReferencia.java`
- `labs/m2/aula-063-stack-heap-referencias/ReferenciaNull.java`
- `labs/m2/aula-063-stack-heap-referencias/ErroNullPointer.java`
- `labs/m2/aula-063-stack-heap-referencias/ValidacaoNull.java`
- `labs/m2/aula-063-stack-heap-referencias/StringImutavel.java`
- `labs/m2/aula-063-stack-heap-referencias/StringParametro.java`
- `labs/m2/aula-063-stack-heap-referencias/StringComRetorno.java`
- `labs/m2/aula-063-stack-heap-referencias/ObjetoMutavel.java`
- `labs/m2/aula-063-stack-heap-referencias/ReatribuirObjetoMutavel.java`
- `labs/m2/aula-063-stack-heap-referencias/ClienteMemoria.java`
- `labs/m2/aula-063-stack-heap-referencias/ClienteReatribuicao.java`
- `labs/m2/aula-063-stack-heap-referencias/ObjetoRetornado.java`
- `labs/m2/aula-063-stack-heap-referencias/ObjetoPerdido.java`
- `labs/m2/aula-063-stack-heap-referencias/ReferenciasCompartilhadas.java`
- `labs/m2/aula-063-stack-heap-referencias/PedidoMemoria.java`
- `labs/m2/aula-063-stack-heap-referencias/ProdutoMemoria.java`
- `labs/m2/aula-063-stack-heap-referencias/PagamentoMemoria.java`
- `labs/m2/aula-063-stack-heap-referencias/OrdemServicoMemoria.java`
- `labs/m2/aula-063-stack-heap-referencias/MensageriaMemoria.java`
- `labs/m2/aula-063-stack-heap-referencias/AuditoriaMemoria.java`
- `labs/m2/aula-063-stack-heap-referencias/DebugStackHeap.java`
- `labs/m2/aula-063-stack-heap-referencias/ErroReferenciaNaoClona.java`
- `labs/m2/aula-063-stack-heap-referencias/ErroReatribuirParametro.java`
- `labs/m2/aula-063-stack-heap-referencias/ErroNullSemValidacao.java`
- `labs/m2/aula-063-stack-heap-referencias/ErroStringSemRetorno.java`
- `labs/m2/aula-063-stack-heap-referencias/README.md`
...
- achar que variável de referência é o objeto;
- achar que atribuir uma referência clona o objeto;
- achar que reatribuir parâmetro troca objeto do chamador;
- alterar objeto sem perceber efeito colateral;
- chamar método em referência `null`;
- verificar `null` depois de chamar método;
- criar objeto e perder referência;
- usar mutação quando retorno seria mais claro;
- não saber se método altera ou calcula;
- confundir ciclo de vida da variável local com ciclo de vida do objeto.

### Próximo passo
Estudar Garbage Collector conceitual.
```
...
```text
1. O que é stack?
2. O que é stack frame?
3. O que é heap?
4. Onde ficam variáveis locais?
5. Onde ficam objetos?
6. O que é uma referência?
7. Qual a diferença entre primitivo e referência?
8. Por que alterar um int dentro do método não muda fora?
9. Por que alterar um array dentro do método muda fora?
10. Qual a diferença entre mutação e reatribuição?
11. O que significa null?
12. Por que NullPointerException acontece?
13. Por que String normalizada dentro do método não muda fora sem retorno?
14. O que acontece quando duas variáveis apontam para o mesmo objeto?
15. Quando um objeto se torna inacessível?
```
